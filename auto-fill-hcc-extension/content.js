...
(() => {
  ...
  const APP_VERSION_LABEL = "1.6 · 26/7"; // hiện ở header panel; đổi tay mỗi lần phát hành (kèm ngày để hỗ trợ)
  ...
  function createPanel(tabId) {
    ...
    title.append(logo, titleText, ver);
  ...
  function handleAutofillMessage(msg, sender, sendResponse) {
    ...
    if (msg?.action === "startFillAllBusiness") {
      ...
      const st = {
        // Người nộp hồ sơ ĐI TRƯỚC (bấm "Sao chép..." để bắt email+sđt tài khoản), rồi các trang còn lại
        // theo CHIỀU XUÔI để Vốn/Thuế nằm cuối (cổng bắt tạo hộ KD trước mới lưu được vốn/thuế).
        order: ["nguoi-nop-ho-so", ...BUSINESS_PAGE_ORDER.filter((k) => k !== "nguoi-nop-ho-so")],
        pages,
        step: 0,
        retries: 0,
        phase: "fill",
    ...
    const resolveFiller = () => formKind === "bacninh"
      ? H.fillFormBacNinh
      : (forceStandard
        ? fillFormStandard
    ...
    // Map formKind → key trên namespace H để chờ engine con nạp xong (race condition document_start).
    ...
    const runFill = async () => {
      ...
      if (typeof filler !== "function") {
        return { error: `Engine điền chưa nạp (formKind=${formKind}).` };
      }
      return filler(fields);
    };
    runFill().then(sendResponse).catch((e) => sendResponse({ error: `Lỗi điền: ${e?.message || e}` }));
  ...
  if (IS_TOP_FRAME && !window.__AUTOFILL_HCC_FILLALL_RESUMED__) {
    ...
    const resume = () => {
      if (typeof H.getFillAllState === "function") {
        H.getFillAllState().then((st) => { if (st) setTimeout(H.stepFillAll, 600); });
      }
      // Phiên đính kèm hộ kinh doanh cũng resume qua từng postback.
      if (typeof H.getAttachAllState === "function") {
        H.getAttachAllState().then((st) => { if (st) setTimeout(H.stepAttachAll, 600); });
      }
    };
  ...
  function isAreaSelectName(name) {
    ...
    return /province|district|village|ward|matinh|maphuongxa|maxa|country_idfld|city_idfld|ward_idfld|street_numberfld|addr[a-z]*ctl/.test(String(name || "").toLowerCase());
  }
  ...
  function formioKeyFromSelect(select, names = []) {
    ...
    for (const raw of candidates) {
      const key = String(raw || "").trim().match(/^data\[([^\]]+)\]$/)?.[1];
      if (key) return key;
    }
    for (const raw of candidates) {
