...
async function handleCopyPersonPage(st, targetKey) {
  ...
  // 2b. Chủ hộ có contact riêng trên hồ sơ. Nút copy tài khoản có thể vừa đổ contact người nộp
  // vào đây, nên phải thay đúng ChuHo_*; hồ sơ không kê khai thì xóa, không giữ nhầm contact tài khoản.
  if (targetKey === "chu-ho-kinh-doanh") {
    const ownerContacts = [
      ["PHONE", "ctl00_C_OWN_PCtl_PERSCtl_PHONEFld"],
      ["EMAIL", "ctl00_C_OWN_PCtl_PERSCtl_EMAILFld"],
    ];
    for (const [kind, id] of ownerContacts) {
      const source = fields.find((f) => new RegExp(`${kind}Fld$`, "i").test(f.name || ""));
      const el = document.getElementById(id);
      if (el) setNativeValue(el, source ? source.value : "", { typing: false, commit: true });
    }
  }

  // 3. Điền ĐỊA CHỈ từ backend — cascade inline (quốc gia→tỉnh→xã→số nhà).
  let addrFields = fields.filter((f) => cfg.addrMatch.test(f.name || ""));
  if (targetKey === "nguoi-nop-ho-so" && Array.isArray(st.businessFlow?.identityCandidates)) {
    const matched = matchCopiedApplicant(st.businessFlow && st.businessFlow.identityCandidates);
    if (matched && matched.diaChi) {
      addrFields = applicantAddressFields(matched.diaChi);
      console.log("[ChangeHKD] địa chỉ người nộp khớp tài khoản:", matched.hoTen, matched.soDinhDanh);
    } else if ((st.businessFlow?.identityCandidates || []).length) {
      console.warn("[ChangeHKD] Không có CCCD nào khớp đồng thời thông tin tài khoản đã sao chép; không tự lấy nhầm địa chỉ.");
      addrFields = [];
    }
  }
