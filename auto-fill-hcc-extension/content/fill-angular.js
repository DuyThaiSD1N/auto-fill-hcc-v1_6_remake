...
(() => {
  ...
  async function fillNgSelect(el, value) {
    ...
    // Backend có thể gửi {value: "<tên để chọn>", other: "<text điền vào ô Khác>"}.
    let selectValue = value;
    let otherText = null;
    if (value && typeof value === "object" && value.value) {
      selectValue = value.value;
      otherText = value.other || null;
    }

    const want = norm(selectValue);
    const fcn = el.getAttribute("formcontrolname") || "";

    // Dân tộc: danh mục của cổng không có đủ mọi tên (vd "Cil", "Cill"). Không khớp được option nào
    // thì tự chọn "Khác" và tự điền nguyên văn vào ô "Dân tộc khác" — người dùng không phải nhập gì.
    const otherFallback = otherText || (/DanToc/i.test(fcn) ? String(selectValue || "") : "");
    ...
    if (cur && ngOptMatch(cur, want)) {
      markFilled(ng);
      // Chỉ gõ ô "… khác" khi option đang chọn ĐÚNG là "Khác"; chọn được option thật thì thôi.
      if (otherFallback && isOtherOption(cur)) await fillOtherTextField(el, otherFallback, ng);
      return true;
    }
    ...
    if (search) {
      search.focus();
      setNativeValue(search, String(selectValue));
    ...
    // Cổng không có option cho giá trị này → quay sang option "Khác" (nếu field có ô text đi kèm).
    let pickedOther = false;
    if (!target && otherFallback) {
      if (search) {
        setNativeValue(search, "Khác");
        search.dispatchEvent(new Event("input", { bubbles: true }));
        await waitFor(() => panelOptions().some((o) => isOtherOption(o.textContent)), 1500);
      }
      target = panelOptions().find((o) => isOtherOption(o.textContent)) || null;
      pickedOther = !!target;
      if (target) {
        console.log(`[AutoFill-NG] [${fcn}] cổng không có option "${selectValue}" → tự chọn "Khác" + tự điền ô "khác"`);
      }
    }

    if (!target) {
      console.warn(`[AutoFill-NG] ng-select [${fcn}] không khớp "${selectValue}". Option:`,
    ...
    // Trigger Angular change detection để form cập nhật UI
    ng.dispatchEvent(new Event("change", { bubbles: true }));
    ng.dispatchEvent(new Event("input", { bubbles: true }));

    markFilled(ng);

    // Chỉ khi thật sự chọn "Khác" mới điền ô text bên cạnh.
    if (pickedOther) await fillOtherTextField(el, otherFallback, ng);

    return true;
  }

  /** Option "Khác" của ng-select — nhận cả biến thể "Khác (ghi rõ)", "Dân tộc khác". */
  function isOtherOption(text) {
    const t = norm(text);
    return t.startsWith("khác") || t.startsWith("khac") || t.includes("dân tộc khác");
  }

  /**
   * Tên control của ô "… khác" đi kèm MỘT dropdown cụ thể, GIỮ NGUYÊN tiền tố Me/Cha.
   * MaDanToc → MaDanTocKhac | DanTocKhac ; MeMaDanToc → MeMaDanTocKhac | MeDanTocKhac ; tương tự Cha.
   * Không bao giờ trả tên của người khác — đó là lý do dân tộc con từng bị điền vào ô của mẹ.
   */
  function otherControlNames(fcn) {
    const prefix = (/^(Me|Cha)/.exec(fcn) || [])[1] || "";
    const rest = fcn.slice(prefix.length);
    const names = [`${fcn}Khac`, `${fcn}Other`];
    if (/^Ma./.test(rest)) names.push(`${prefix}${rest.slice(2)}Khac`);
    return names;
  }

  // Điền văn bản vào ô "… khác" bên cạnh dropdown. CHỈ tìm trong phạm vi của CHÍNH field đó:
  // trước đây quét toàn form nên giá trị hay rơi sang ô của người khác (dân tộc con → ô của mẹ).
  async function fillOtherTextField(parentEl, text, ngSelectEl) {
    // Chờ lâu hơn để Angular render ô "Dân tộc khác"
    await sleep(800);

    console.log(`[AutoFill-NG] Tìm ô "Dân tộc khác" để điền: "${text}"`);

    const fcn = parentEl.getAttribute('formcontrolname') || '';
    const searchBox = ngSelectEl && ngSelectEl.querySelector('input[type="text"]');
    const usable = (inp) => !!inp && inp !== searchBox
      && inp.getAttribute('role') !== 'combobox' && isVisible(inp);

    let otherInput = null;

    // (1) Đúng control của CHÍNH field này (giữ tiền tố Me/Cha) — chắc chắn không sang người khác.
    for (const name of otherControlNames(fcn)) {
      const wrapper = document.querySelector(`[formcontrolname="${name}"]`);
      if (!wrapper) continue;
      const inp = wrapper.tagName === 'INPUT' ? wrapper : wrapper.querySelector('input[type="text"]');
      if (usable(inp)) { otherInput = inp; break; }
    }

    // (2) Ô text nằm ngay trong khối của dropdown.
    if (!otherInput) {
      otherInput = Array.from(parentEl.querySelectorAll('input[type="text"]')).find(usable) || null;
    }

    // (3) Khối liền kề — cổng hay render ô "khác" thành app-input đứng ngay sau dropdown.
    if (!otherInput && parentEl.nextElementSibling) {
      otherInput = Array.from(parentEl.nextElementSibling.querySelectorAll('input[type="text"]')).find(usable) || null;
    }

    // (4) Ô có placeholder/aria-label "khác" trong CÙNG hàng (không quét cả form như trước).
    if (!otherInput && parentEl.parentElement) {
      otherInput = Array.from(parentEl.parentElement.querySelectorAll('input[type="text"]')).find((inp) => {
        const hint = `${inp.getAttribute('placeholder') || ''} ${inp.getAttribute('aria-label') || ''} ${inp.getAttribute('data-placeholder') || ''}`.toLowerCase();
        return (hint.includes('khác') || hint.includes('other')) && usable(inp);
      }) || null;
    }

    if (otherInput) {
      setNativeValue(otherInput, String(text));
      otherInput.dispatchEvent(new Event("input", { bubbles: true }));
      otherInput.dispatchEvent(new Event("change", { bubbles: true }));
      otherInput.dispatchEvent(new Event("blur", { bubbles: true }));
      const box = otherInput.closest("mat-form-field") || otherInput;
      markFilled(box);
      console.log(`[AutoFill-NG] ✅ [${fcn}] đã điền ô "khác": "${text}"`, otherInput);
      return true;
    } else {
      // Debug: liệt kê tất cả inputs visible trong form
      const allInputs = Array.from(document.querySelectorAll('input[type="text"]')).filter(inp => isVisible(inp));
      console.warn(`[AutoFill-NG] ❌ [${fcn}] không thấy ô "khác" để điền "${text}" — bỏ trống, KHÔNG ghi sang ô khác`);
      console.warn(`[AutoFill-NG] Parent element:`, parentEl);
      console.warn(`[AutoFill-NG] Tất cả ${allInputs.length} text inputs visible trong form:`,
        allInputs.map(inp => ({
          tag: inp.tagName,
          role: inp.getAttribute('role'),
          formcontrolname: inp.closest('[formcontrolname]')?.getAttribute('formcontrolname'),
