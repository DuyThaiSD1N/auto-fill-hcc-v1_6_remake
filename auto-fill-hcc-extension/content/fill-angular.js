...
(() => {
  ...
  async function fillNgSelect(el, value) {
    ...
    const appendTo = ng.getAttribute("appendto") || ng.getAttribute("appendTo") || "";
    console.log(`[AutoFill-NG] fillNgSelect [${fcn}] value="${value}" appendTo="${appendTo}"`);

    // (1) Đã chọn sẵn đúng giá trị → coi như thành công.
    ...
    // Nếu appendTo="body": panel render ở body, không nằm trong ng, phải đóng panel khác đang mở trước.
    if (appendTo) {
      // Đóng panel đang mở (nếu có) tránh conflict
      document.querySelectorAll("ng-dropdown-panel").forEach(p => {
    ...
    await waitFor(() => panelOptions().length > 0 || ng.querySelector("input[type=text]"), 1500);
    if (!panelOptions().length && !ng.querySelector("input[type=text]")) {
      ctrl.dispatchEvent(new MouseEvent("mousedown", { bubbles: true }));
      await sleep(300);
    }
    ...
    if (search) {
      ...
      await sleep(500); // chờ ng-select filter/load option từ API (tăng từ 300→500ms)
    }

    // (4) Chờ option thật (bỏ "không có"/"đang tải") — tối đa 5s cho danh mục lazy-load từ server.
    await waitFor(() => {
    ...
    // Khớp chính xác trước, sau đó khớp mờ (includes)
    let target = opts.find((o) => norm(o.textContent) === want)
      || opts.find((o) => ngOptMatch(o.textContent, want));
    if (!target) {
      console.warn(`[AutoFill-NG] ng-select [${fcn}] không khớp "${value}". Options (${opts.length}):`,
        opts.map((o) => o.textContent.trim()).filter(Boolean).slice(0, 30));
      // Đóng panel sạch: gõ trống rồi click backdrop
      if (search) setNativeValue(search, "");
      const bd = document.querySelector(".ng-dropdown-panel");
      if (bd) bd.dispatchEvent(new MouseEvent("mousedown", { bubbles: true }));
      ng.dispatchEvent(new MouseEvent("mousedown", { bubbles: true }));
      return false;
    }
