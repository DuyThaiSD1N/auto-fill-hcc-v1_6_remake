...
async def ocr_per_file(files: list[dict], provider: str | None = None) -> list[dict]:
    """OCR từng file, CÓ CACHE theo hash nội dung (bù OCR lại cùng file ở bước đính kèm).

    Đọc cache 1 query $in cho cả lô → file HIT trả text cache (không OCR), file MISS mới OCR
    thật rồi ghi cache CHẠY NỀN. Thứ tự kết quả giữ nguyên như input. Tắt cache → chạy thẳng.
    """
    from app.services import ocr_cache

    if not settings.ocr_cache_enabled or not files:
        return await _ocr_per_file_uncached(files, provider)
    ...
    for f, k in zip(files, keys):
        if k and k in cached:
            ...
            r = next(fresh_iter)
            results.append(r)
            if k and not r.get("error") and (r.get("text") or "").strip():
                to_cache.append((k, r["text"], r.get("provider")))

    ocr_cache.put_many_bg(to_cache)
    return results


async def _ocr_per_file_uncached(files: list[dict], provider: str | None = None) -> list[dict]:
    """OCR từng file (không cache). Nếu file mang cờ `hasHandwriting` (mỗi tài liệu chọn viết tay
    riêng) thì ROUTE THEO TỪNG FILE: viết tay → Vintern, còn lại → raw. Không có cờ nào → dùng 1
    provider (context/tham số, tương thích cũ). Mỗi result gắn thêm khóa "provider".
    """
