...
async def _submitter_context(documents: list[dict], options: dict) -> str:
    """Neo NGƯỜI NỘP (tên+CCCD cổng tự đổ từ tài khoản vào Phần I) để LLM tách khỏi ĐỐI TƯỢNG hưởng trợ cấp.

    Thủ tục có thể NỘP THAY / KHAI THAY: người khác đứng nộp cho đối tượng và có thể UPLOAD CẢ CCCD của mình
    (mục 5 checklist online yêu cầu "CCCD của người khai thay"). Không có mỏ neo, LLM dễ vơ CCCD người nộp
    làm đối tượng. Mỏ neo giúp LLM biết CCCD nào là của người nộp để trích NguoiNop_*; đối tượng lấy từ Tờ
    khai (Mẫu 1a-1đ/2a/2b/03) / Giấy xác nhận khuyết tật / Biên bản giám định / Giấy khai sinh / CCCD của họ.
    """
    ...
    if matched:
        ...
        return (
            ...
            "- ĐỐI TƯỢNG hưởng trợ cấp (chủ hồ sơ) = người đứng tên trên Tờ khai (Mẫu 1a-1đ/2a/2b/03) / Giấy "
            "xác nhận khuyết tật / Biên bản giám định / Giấy khai sinh. Trích vào các field DoiTuong_* TỪ các "
            f"giấy tờ đó, TUYỆT ĐỐI KHÔNG lấy nhân thân đối tượng từ CCCD người nộp (tài liệu #{idx}).\n"
            "- NẾU người nộp CHÍNH LÀ đối tượng (cùng họ tên hoặc cùng số định danh với Tờ khai) → chỉ 1 người: "
            f"cả DoiTuong_* lẫn NguoiNop_* lấy từ CCCD #{idx} + Tờ khai.\n"
    ...
    return (
        ...
        "DoiTuong_* (đối tượng hưởng trợ cấp = chủ hồ sơ) trích bình thường từ Tờ khai / Giấy xác nhận khuyết "
        "tật / Biên bản giám định / Giấy khai sinh / CCCD của đối tượng. Bỏ trống NguoiNop_*.\n"
        "</nguoi_nop_context>"
    )
