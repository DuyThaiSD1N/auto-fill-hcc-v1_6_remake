"""Map facts nguồn sang Form.io của cổng DVC Quảng Ngãi (giao/thuê/chuyển mục đích sử dụng đất)."""
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    # Tên chủ hồ sơ khác tên người nộp => hồ sơ có ủy quyền (không có ô checkbox "người nộp là chủ hồ
    # sơ" trên cổng Quảng Ngãi nên đây là tín hiệu duy nhất).
    authorized = bool(owner_name and applicant_name and _fold(owner_name) != _fold(applicant_name))

    # KHÁC hẳn cổng Ninh Bình: form Quảng Ngãi KHÔNG có khối riêng cho người nộp. Panel "Thông tin
    ...
    # KHỐI THỬA ĐẤT (data[diaChiThuaDat]/province2/village2/nation2) là ĐỊA ĐIỂM KHU ĐẤT, tách hẳn
    # khối địa chỉ cư trú ở trên. Chỉ phát khi đọc được nguồn riêng ThuaDat_DiaChi — KHÔNG bao giờ
    # lấy applicant_area thay thế, vì nơi ở và thửa đất thường khác nhau.
