"""Map compact source facts → Form.io data[...] fields cho "Đăng ký đất đai, cấp GCN QSDĐ lần đầu" (cổng
Đà Nẵng). CÙNG contact block + panel thửa đất với #136 giao_thue — chỉ khác _PROC_TITLE + nguồn (Đơn M15).

HAI vai trong 1 panel:
- CHỦ HỒ SƠ (người đề nghị đăng ký/cấp GCN, hoặc người được cử đại diện) → data[ownerFullname]
...
- data[noidungyeucaugiaiquyet] = nội dung đề nghị (chép đầy đủ Đơn M15) hoặc câu khung khi trống.
...
from app.pipelines.dang_ky_dat_dai_lan_dau_da_nang.process.schema import UI_COMP_BY_NAME

# Văn bản "Nội dung yêu cầu giải quyết" fallback: "{tên chủ hồ sơ} ĐỀ NGHỊ GIẢI QUYẾT {tên thủ tục}".
_PROC_TITLE = (
    "đăng ký đất đai, tài sản gắn liền với đất, cấp Giấy chứng nhận quyền sử dụng đất, quyền sở hữu tài "
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    if not owner_name:
        warnings.append("Thiếu tên chủ hồ sơ (người đề nghị đăng ký, cấp GCN lần đầu).")
    ...
    # Nội dung yêu cầu: chép ĐẦY ĐỦ nội dung đề nghị từ Đơn Mẫu 15 (GIỮ xuống dòng); CHỈ khi trống mới
    ...
    if is_uy_quyen:
        ...
        add("data[identityDate]", _date(values.get("NguoiNop_NgayCap")))
        add("data[identityAgency]", _issuer(values.get("NguoiNop_NoiCap")))
        # Tự nộp: địa chỉ người nộp = địa chỉ chủ hồ sơ. ƯU TIÊN ChuHoSo_DiaChi (lấy từ đơn/tờ khai — đáng
        # tin hơn CCCD hay đọc lệch/tên đơn vị cũ), thiếu mới lấy NguoiNop_DiaChi.
