"""Map facts nguồn sang Form.io của cổng DVC Quảng Ngãi (đính chính GCN đã cấp lần đầu có sai sót)."""
...
# Bảng thành phần hồ sơ có ĐÚNG 4 dòng (Đơn Mẫu số 18 / Bản gốc GCN đã cấp / Giấy tờ chứng minh sai
# sót / Văn bản ủy quyền) — KHÔNG có dòng riêng cho giấy xác nhận cư trú, tờ khai thuế hay trích lục
# hồ sơ đất. Ảnh ánh xạ ANH_XA_Thanh_phan_ho_so_eForm_Dinh_chinh_GCN chốt: CCCD/giấy tờ tùy thân đi
# vào dòng "Giấy tờ chứng minh sai sót" (đó chính là chứng cứ so sánh tên đúng ↔ tên sai trên GCN),
# các giấy tờ còn lại đi chung dòng Đơn. Ghi chú này để cán bộ tiếp nhận biết chỗ tìm trong file PDF
# gộp và biết có thể tự bấm "+ Thêm giấy tờ" để tách nếu cổng yêu cầu.
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    # Cổng Quảng Ngãi KHÔNG có khối riêng cho người nộp. Nhãn đầu khối ghi rõ "Thông tin chủ hồ sơ"
    # nên ngày sinh, giới tính, email, số điện thoại, số định danh, ngày/nơi cấp và nhóm địa chỉ đều
    # là thông tin CHỦ HỒ SƠ. Về người nộp chỉ có đúng 2 ô: "Họ và tên người nộp hồ sơ"
    # (data[fullname]) và "Số điện thoại ủy quyền" (data[phoneNumber1]). Vì vậy KHÔNG được đổi nguồn
    # khối này sang NguoiNop_* khi có ủy quyền — làm thế là ghi nhân thân người được ủy quyền vào ô
    # của chủ hồ sơ.
    ...
    # CŨ/khác tỉnh (địa danh trước sáp nhập) nên tuyệt đối không để nó ghi đè địa chỉ trên đơn.
    owner_area = (
    ...
    # không tồn tại việc ủy quyền nào để mà ghi số (file MAPPING cũng ghi "để trống — hồ sơ KHÔNG có
    # văn bản ủy quyền").
    if authorized:
        add("data[phoneNumber1]", _phone(values.get("NguoiNop_DienThoai")))
    add("data[chonDoiTuong]", "Cá nhân", default=True)
    # Địa chỉ trong khối "Thông tin chủ hồ sơ" cũng là nơi cư trú CHỦ HỒ SƠ (xem giải thích ở trên).
    ...
    # KHÔNG phát khối "Địa chỉ thửa đất/ địa chỉ xây dựng" (data[diaChiThuaDat]/province2/village2/
    # nation2): panel đó mặc định THU GỌN nên Form.io không render 4 ô bên trong (grep trang điền
    # 'đính chính fill.html' = 0 hit). 4 ô cũng không có trong UI_COMP_BY_NAME nên không có đường nào
    # nhét nơi cư trú hay địa chỉ thửa đất trên Giấy chứng nhận vào.
