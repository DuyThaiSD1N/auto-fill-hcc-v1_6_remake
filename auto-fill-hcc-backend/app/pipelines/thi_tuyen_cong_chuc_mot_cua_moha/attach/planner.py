"""Đính kèm cho thủ tục Thi tuyển công chức.

Form bước "Thành phần hồ sơ" của nhóm thủ tục này là bảng ô cố định. Planner chỉ
đính file được nhận diện là Phiếu đăng ký dự tuyển vào slot tương ứng; giấy tờ
định danh/tài liệu khác được bỏ qua để tránh đính nhầm.
"""
...
from app.pipelines.thi_tuyen_cong_chuc_mot_cua_moha.attach import prompt
...
SLOT = {
    ...
    "slotIndex": 0,
    "slotName": "Phiếu đăng ký dự tuyển/thi tuyển công chức",
...
def detect_slot_key(text: str, file_name: str = "") -> str:
    """Rule fallback: nhận diện Phiếu đăng ký dự tuyển công chức từ OCR text."""
    _ = file_name
    haystack = _fold(text or "")
    if not haystack:
        return ""
    has_registration_form = "phieu dang ky du tuyen" in haystack or "phieu dang ky thi tuyen cong chuc" in haystack
    if has_registration_form and (
        "cong chuc" in haystack or "nghi dinh so 170/2025" in haystack or "vi tri viec lam du tuyen" in haystack
    ):
        return SLOT["slotKey"]
    if (
        ("mau so 01" in haystack or "mau so 02" in haystack)
        and "vi tri viec lam du tuyen" in haystack
        and "co quan to chuc don vi du tuyen" in haystack
    ):
        return SLOT["slotKey"]
    if "nghi dinh so 170/2025" in haystack and "du tuyen" in haystack:
        return SLOT["slotKey"]
    return ""
    ...
    for idx, file in enumerate(files):
        ...
        if slot_key != SLOT["slotKey"]:
            if is_excluded_document(text, file_name):
                warnings.append(f"File '{file_name}' là giấy tờ/tài liệu khác — không đính ở ô Phiếu đăng ký dự tuyển.")
            else:
...
async def plan(
    ...
    """Entry point đính kèm cho thi-tuyen-cong-chuc."""
    _ = session
    _ = options or {}
    errors: list[str] = []
