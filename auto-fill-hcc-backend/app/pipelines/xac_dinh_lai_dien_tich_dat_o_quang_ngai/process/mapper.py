"""Map facts nguồn sang Form.io của cổng DVC Quảng Ngãi (xác định lại diện tích đất ở)."""
...
# Bảng thành phần hồ sơ chỉ có 3 dòng THẬT (Giấy chứng nhận đã cấp / Đơn Mẫu số 11/ĐK / Văn bản về
# việc đại diện) — KHÔNG có dòng riêng cho CCCD, giấy xác nhận cư trú hay văn bản của cơ quan đăng ký
# đất đai. Planner gom chúng vào dòng Đơn; ghi chú này để cán bộ tiếp nhận biết chỗ tìm trong file
# PDF gộp và biết có thể tự bấm "+ Thêm giấy tờ" để tách nếu cổng yêu cầu.
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    # ĐỊA CHỈ: ưu tiên TẤT ĐỊNH địa chỉ ghi trên ĐƠN/TỜ KHAI (field nguồn riêng Don_DiaChi, LLM chỉ
    # được lấy từ đơn). Chỉ khi đơn không ghi mới rơi về nơi cư trú suy từ giấy tờ tùy thân — CCCD hay
    # ghi nơi thường trú CŨ/khác tỉnh nên không được phép ghi đè địa chỉ người dân tự khai trên đơn.
    ...
    # nation2): bản dựng form của thủ tục này chỉ còn HEADER panel đó, Form.io không render 4 ô bên
    # trong (grep 'fill .html' = 0 hit). 4 ô cũng không có trong UI_COMP_BY_NAME nên không có đường
    # nào nhét nơi cư trú hay địa chỉ thửa đất trên Giấy chứng nhận vào.
