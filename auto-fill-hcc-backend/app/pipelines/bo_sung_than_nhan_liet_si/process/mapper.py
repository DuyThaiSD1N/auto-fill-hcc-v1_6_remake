"""Map fact Đơn Mẫu 26 + CCCD + hồ sơ liệt sĩ sang field Form.io của thủ tục Bổ sung thân nhân LS.

- Field dùng chung Phần 1 (người nộp) và Phần 4 (chi tiết mẫu khai) điền cùng formcontrolname
  nhưng khác occurrence (0 = Phần 1, 1 = Phần 4).
- Phần 4: Quê quán = province/district/address (occ 1); Nơi thường trú = province1/district1/address1.
- Cư trú/quê quán ưu tiên TỜ KHAI (đơn vị hành chính hiện hành); số định danh/ngày cấp ưu tiên CCCD.
"""
...
from app.pipelines.bo_sung_than_nhan_liet_si.process.schema import UI_COMP_BY_NAME
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    if not candidate_name or not candidate_identity:
        warnings.append("Thiếu họ tên hoặc số định danh người khai từ Đơn Mẫu 26/CCCD.")
    ...
    # Quê quán → province/district/address (occ 1).
    _add_area(add, "data[province]", "data[district]", "data[address]", values.get("ToKhai_QueQuan"), occurrence=1)
    # Nơi thường trú → province1/district1/address1.
    _add_area(add, "data[province1]", "data[district1]", "data[address1]", candidate_residence)
    add("data[QuanHe]", _text(values.get("ToKhai_QuanHeVoiLietSi")))
    ...
    add("data[CoQuanDonVi]", _text(values.get("LietSi_CoQuanDonVi")))
    add("data[ChucVu]", _text(values.get("LietSi_CapBac")))
    add("data[so]", _text(values.get("LietSi_SoBang")))
    add("data[QD]", _text(values.get("LietSi_SoQuyetDinh")))
    add("data[ngaycap]", _date(values.get("LietSi_NgayQuyetDinh")))

    # --- Bảng thân nhân đề nghị bổ sung (dataGrid) ---
    than_nhan = _list(values.get("ToKhai_ThanNhan"))
    for idx, item in enumerate(than_nhan[:8]):
        add(f"data[dataGrid][{idx}][textField1]", _item_text(item, "hoTen", "hoVaTen"))
        add(f"data[dataGrid][{idx}][NgayHiSinh1]", _date(_item_text(item, "ngaySinh", "ngayThangNamSinh")))
        add(f"data[dataGrid][{idx}][textField2]", _item_text(item, "soGiayTo", "soCccd", "soGiayKhaiSinh"))
        add(f"data[dataGrid][{idx}][textField3]", _item_text(item, "moiQuanHe", "mqh", "quanHe"))
        add(f"data[dataGrid][{idx}][textField6]", _item_text(item, "noiThuongTru", "diaChi"))
        add(f"data[dataGrid][{idx}][textField5]", _item_text(item, "hoanCanh"))

    # --- Khai báo hồ sơ đính kèm (dòng đầu) ---
    add("data[hoSoDinhKem][0][textField1]", "Đơn đề nghị sửa đổi, bổ sung thông tin trong hồ sơ liệt sĩ")
    add("data[hoSoDinhKem][0][textField2]", "Bản chính")

    if not values.get("LietSi_HoTen"):
        warnings.append("Chưa đọc được họ tên liệt sĩ trong Đơn Mẫu 26.")
    if not than_nhan:
        warnings.append("Chưa đọc được danh sách thân nhân đề nghị bổ sung trong Đơn Mẫu 26.")
    return out, warnings
