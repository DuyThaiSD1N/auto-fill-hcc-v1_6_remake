"""Đính kèm cho [Lào Cai] Chuyển mục đích sử dụng đất (1.115651).

LLM đọc OCR từng file → nhãn rút gọn (catalog.py) + nhóm hồ sơ. Nhóm lấy theo ĐƠN có trong hồ sơ (Mẫu 02/03/17/18);
không có đơn thì theo nhóm LLM suy ra; không rõ → nhóm (1) chuyển mục đích. BE đổi nhãn thành:
  - ô fixed-slot của đúng dòng trong nhóm (FE khoanh vùng theo tiêu đề nhóm, tích checkbox dòng rồi bơm tệp);
  - hoặc một dòng "Giấy tờ khác" (target=new, tên = documentName) cho giấy tờ không có dòng trong nhóm: bản đồ riêng,
    GCN đăng ký doanh nghiệp, giấy ủy quyền, đơn của nhóm khác, tài liệu khác.
...
def _pick_group(entries: list[dict], llm_group: str) -> tuple[str, str]:
    forms = [catalog.FORM_GROUP[e["label"]] for e in entries if e["label"] in catalog.FORM_GROUP]
    if forms:
        # Nhiều đơn khác nhóm hiếm gặp → lấy đơn xuất hiện đầu tiên, các đơn còn lại vào Giấy tờ khác.
        return forms[0], "form"
