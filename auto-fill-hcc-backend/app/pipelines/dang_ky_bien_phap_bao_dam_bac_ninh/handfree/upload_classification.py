"""Phân loại tệp lúc upload (checklist handfree) cho thủ tục Đăng ký biện pháp bảo đảm
bằng quyền sử dụng đất, tài sản gắn liền với đất — tỉnh Bắc Ninh (1.011441).

Chỉ là lớp xếp tệp vào checklist khi công dân tải ảnh/scan. Phân loại để điền đơn và
định tuyến thành phần hồ sơ trên cổng Bắc Ninh vẫn do ``attach/planner.py`` quyết định
từ toàn bộ OCR.
"""
import json
import re
...
PROCEDURE_KEYS = frozenset({"dang-ky-bien-phap-bao-dam-bac-ninh"})
_ALLOWED_KEYS = frozenset({"don", "hop_dong", "gcn", "cccd", "khac"})

_SYSTEM_PROMPT = """
Bạn là bộ phân loại giấy tờ cho thủ tục Đăng ký biện pháp bảo đảm bằng quyền sử dụng đất,
tài sản gắn liền với đất (đăng ký thế chấp đất đai).
...
- don: PHIẾU YÊU CẦU đăng ký biện pháp bảo đảm (Mẫu số 01a). Tiêu đề có "PHIẾU YÊU CẦU
  ĐĂNG KÝ" và nhắc "biện pháp bảo đảm"; nội dung khai bên bảo đảm, bên nhận bảo đảm,
  hợp đồng bảo đảm, mô tả tài sản (thửa đất, Giấy chứng nhận).
- hop_dong: HỢP ĐỒNG thế chấp / hợp đồng bảo đảm quyền sử dụng đất (thường ký giữa bên
  thế chấp và ngân hàng, có thể kèm lời chứng công chứng). Tiêu đề có "HỢP ĐỒNG THẾ CHẤP"
  hoặc "HỢP ĐỒNG" gắn với tài sản bảo đảm.
- gcn: GIẤY CHỨNG NHẬN quyền sử dụng đất, quyền sở hữu tài sản gắn liền với đất (sổ đỏ /
  sổ hồng). Có số phát hành, số vào sổ cấp GCN, thông tin thửa đất, tờ bản đồ.
...
- khac: giấy tờ liên quan còn lại — Giấy chứng nhận đăng ký doanh nghiệp, giấy giới thiệu,
  văn bản ủy quyền, biên bản, giấy tờ khác.
...
số Giấy chứng nhận hoặc số căn cước; hợp đồng vẫn là hop_dong dù mô tả thửa đất. TUYỆT ĐỐI
không phân loại thành cccd chỉ vì tài liệu nhắc đến "CCCD" hoặc có 12 chữ số. Chỉ dùng
OCR_TEXT làm bằng chứng; không dùng tên file.
...
def fallback_to_slot(
    ...
    if ("phieu yeu cau dang ky" in folded or "mau so 01a" in folded
            or ("phieu yeu cau" in folded and "bien phap bao dam" in folded)) and free("don"):
        return "don", None, "fallback theo tiêu đề Phiếu yêu cầu 01a"

    if ("hop dong the chap" in folded or "hop dong bao dam" in folded
            or ("hop dong" in folded and "the chap" in folded)) and free("hop_dong"):
        return "hop_dong", None, "fallback theo tiêu đề hợp đồng bảo đảm"
    ...
    if any(marker in folded for marker in (
        "giay chung nhan dang ky doanh nghiep", "giay gioi thieu",
        "van ban uy quyen", "giay uy quyen",
    )) and free("khac"):
        return "khac", None, "fallback theo giấy tờ pháp lý kèm theo"
