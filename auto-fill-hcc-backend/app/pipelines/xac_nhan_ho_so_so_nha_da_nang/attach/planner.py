"""Đính kèm bước "Thành phần hồ sơ" cho thủ tục "Xác nhận hồ sơ cấp mới/cấp lại Giấy chứng nhận biển số
nhà" (cổng DVC Đà Nẵng — Angular mat-table, engine FE `attp-row`, CÙNG cổng #68/#75/#110).

Bảng CHỈ CÓ 1 DÒNG: "Đơn đề nghị cấp giấy chứng nhận số nhà" — Bản chính. Cổng chỉ yêu cầu duy nhất 1
giấy tờ. Giấy chứng nhận QSDĐ được đính kèm THÊM vào chính dòng này như minh chứng (engine attp-row gộp
nhiều file/1 dòng theo componentName). Vì vậy _DON và _GCN cùng trỏ về 1 dòng. CCCD → bỏ qua.
...
from app.pipelines.xac_nhan_ho_so_so_nha_da_nang.attach import prompt
...
# DÒNG DUY NHẤT trên bảng. componentName = ĐOẠN TEXT ĐẶC TRƯNG (FE khớp substring fold). Đơn (bản chính) +
# GCN QSDĐ (minh chứng) đều đổ vào dòng này → cùng componentName để engine gộp file. loaiBan cả 2 = Bản chính.
_SINGLE_ROW = {
    "componentName": "Đơn đề nghị cấp giấy chứng nhận số nhà",
    "componentIndex": 1,
    "loaiBan": "Bản chính",
    "documentName": "Đơn đề nghị cấp giấy chứng nhận số nhà",
}
_ROWS: dict[str, dict[str, Any]] = {
    _DON: _SINGLE_ROW,
    _GCN: _SINGLE_ROW,  # GCN QSDĐ đính kèm minh chứng vào cùng dòng Đơn (cổng chỉ có 1 ô).
}
