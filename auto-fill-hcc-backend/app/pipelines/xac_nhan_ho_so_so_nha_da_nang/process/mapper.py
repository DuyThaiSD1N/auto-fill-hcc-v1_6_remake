"""Map compact source facts → Form.io data[...] fields cho "Xác nhận hồ sơ cấp mới/cấp lại GCN biển số
nhà" (cổng Đà Nẵng). Field-key data[...] Y HỆT cap_gcn_so_nha_da_nang (#110) — chỉ khác tên thủ tục.

HAI vai trong 1 panel:
- CHỦ HỒ SƠ (chủ sở hữu nhà, người ký đơn) → data[ownerFullname] (+ data[organization] khi tổ chức).
...
- data[isOwnerDossier]: True khi chủ hồ sơ = người nộp (tự nộp — mặc định của số nhà); False khi ỦY QUYỀN.
- data[noidungyeucaugiaiquyet] = "{tên chủ hồ sơ} ĐỀ NGHỊ GIẢI QUYẾT {tên thủ tục}[ tại địa chỉ {…}]".
"""
...
from app.pipelines.xac_nhan_ho_so_so_nha_da_nang.process.schema import UI_COMP_BY_NAME

# "Nội dung yêu cầu giải quyết" ghép: "{tên chủ hồ sơ} ĐỀ NGHỊ GIẢI QUYẾT {tên thủ tục}[ tại địa chỉ …]".
# Tên thủ tục lấy ĐÚNG mẫu prefill trên form (breadcrumb) để khớp giá trị hệ thống.
_PROC_TITLE = "Thủ tục xác nhận hồ sơ cấp mới/cấp lại Giấy chứng nhận biển số nhà"
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    if not owner_name:
        warnings.append("Thiếu tên chủ hồ sơ (chủ sở hữu nhà đề nghị cấp số nhà).")

    # ỦY QUYỀN nếu người nộp KHÁC chủ hồ sơ; TỰ NỘP nếu trùng (hoặc thiếu người nộp) — mặc định số nhà.
    ...
    if owner_name:
        noi_dung = f"{owner_name} ĐỀ NGHỊ GIẢI QUYẾT {_PROC_TITLE}"
        if dia_chi_xin_cap:
            noi_dung += f" tại địa chỉ {dia_chi_xin_cap}"
