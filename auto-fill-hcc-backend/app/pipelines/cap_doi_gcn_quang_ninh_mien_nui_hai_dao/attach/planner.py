"""Lập kế hoạch đính kèm hồ sơ CẤP ĐỔI Giấy chứng nhận QSDĐ (miền núi, hải đảo) tại Quảng Ninh.

Cùng nền tảng (React/Radix, modal "Danh sách tài liệu điện tử" → engine wallet-modal, target "existing")
với đăng ký biến động Quảng Ninh, nhưng danh mục thành phần hồ sơ NGẮN hơn (6 hàng). componentName là
khóa chính vì FE khớp substring sau khi fold dấu; componentIndex (1-based) chỉ là gợi ý khi bảng đúng thứ tự.
"""
