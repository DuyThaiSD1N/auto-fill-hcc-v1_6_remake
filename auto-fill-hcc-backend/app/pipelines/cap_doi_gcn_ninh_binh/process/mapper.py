"""Map facts nguồn sang Form.io của cổng DVC Ninh Bình (cấp đổi Giấy chứng nhận QSDĐ, tài sản gắn liền với đất)."""
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    # điền cả ownerFullname lẫn organization) để form không bỏ trống tên cơ quan.
    ...
    add("data[noidungyeucaugiaiquyet]", _text(values.get("Don_NoiDungCapDoi")))
    # Ô Ghi chú: nêu rõ cách đính kèm (form cấp đổi chỉ có 2 dòng, dòng 2 gộp Đơn + GCN + ủy quyền + CCCD).
    add(
        "data[note]",
        "Giấy chứng nhận đã cấp, văn bản ủy quyền và CCCD được đính kèm chung tại dòng Đơn đăng ký "
        "biến động đất đai (Mẫu số 11/ĐK).",
        default=True,
    )
