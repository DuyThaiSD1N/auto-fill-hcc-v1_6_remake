"""Ánh xạ facts → ô eForm của thủ tục [Lào Cai] đăng ký đất đai, cấp GCN lần đầu (hộ gia đình, cá nhân).
...
⚑ Trang "Thành phần hồ sơ" có 2 textarea nhưng mapper CHỈ phát `HoSoOnline_ghiChu`: nội dung cổng tự
điền sẵn ở ô "Về việc" đã đúng tên thủ tục, nghiệp vụ yêu cầu GIỮ NGUYÊN — ghi đè là làm sai lệch.
"""
...
def _ghi_chu(values: dict) -> str | None:
    """Ô "Ghi chú" — mô tả hồ sơ để cán bộ tiếp nhận đối chiếu nhanh.

    Ghép tất định từ nhân thân chủ hồ sơ + thửa đất. Không đủ tên chủ hồ sơ hoặc số thửa thì không
    phát field (câu mô tả cụt không giúp được gì).

    Danh mục tệp đính kèm KHÔNG ghép ở đây: bước process chưa biết kết quả phân loại tệp — planner
    phát cảnh báo để cán bộ bổ sung, nhất là khi có tệp đính kèm thay thế.
    """
    ho_ten = _plain(values.get("ChuHoSo_HoTen")) or _plain(values.get("ChuHoSo_TenToChuc"))
    so_thua = _plain(values.get("ThuaDat_SoThua"))
    if not ho_ten or not so_thua:
        return None

    nhan_than = [f"Hồ sơ của {ho_ten}"]
    ngay_sinh = _date(values.get("ChuHoSo_NgaySinh"))
    if ngay_sinh:
        nhan_than.append(f"sinh ngày {ngay_sinh}")
    so_dinh_danh = _digits(values.get("ChuHoSo_SoDinhDanh"))
    if so_dinh_danh:
        nhan_than.append(f"số định danh {so_dinh_danh}")

    thua = [f"thửa đất số {so_thua}"]
    ...
    muc_dich = _plain(values.get("ThuaDat_MucDichSuDung"))
    if muc_dich:
        thua.append(f"mục đích sử dụng {muc_dich}")
    ...
    return "; ".join([", ".join(nhan_than), ", ".join(thua)]) + "."
