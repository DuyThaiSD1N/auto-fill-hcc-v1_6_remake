"""Compact agent process pipeline cho "Sửa đổi, bổ sung thông tin cá nhân trong hồ sơ người có công"."""

import re
import unicodedata

from app.pipelines._shared.compact_agent import runner
from app.pipelines.sua_doi_thong_tin_ho_so_nguoi_co_cong.process import mapper
from app.pipelines.sua_doi_thong_tin_ho_so_nguoi_co_cong.process.prompt import EXTRA_RULES
from app.pipelines.sua_doi_thong_tin_ho_so_nguoi_co_cong.process.schema import (
    ALIASES,
    ALLOWED,
...
async def _submitter_context(documents: list[dict], options: dict) -> str:
    """Neo NGƯỜI NỘP (tên+CCCD cổng tự đổ từ tài khoản vào Phần I) để LLM tách khỏi NGƯỜI KHAI ĐƠN.

    Thủ tục có thể NỘP THAY: người khác đứng nộp cho người khai đơn và có thể UPLOAD CẢ CCCD của mình.
    Không có mỏ neo, LLM dễ vơ CCCD người nộp làm người khai (NguoiKhai_*). Mỏ neo giúp LLM biết CCCD nào
    là của người nộp để trích NguoiNop_*; người khai đơn lấy từ Đơn Mẫu 26 / Bản khai / CCCD của họ.
    """
    ...
    if matched:
        ...
        return (
            ...
            "- NGƯỜI KHAI ĐƠN (chủ hồ sơ) = người đứng tên trên Đơn đề nghị Mẫu số 26 / Bản khai. Trích thông "
            "tin người khai vào các field NguoiKhai_* TỪ các giấy tờ đó, TUYỆT ĐỐI KHÔNG lấy nhân thân người "
            f"khai từ CCCD người nộp (tài liệu #{idx}).\n"
            "- NẾU người nộp CHÍNH LÀ người khai (cùng họ tên hoặc cùng số định danh với Đơn Mẫu 26) → chỉ 1 "
            f"người: cả NguoiKhai_* lẫn NguoiNop_* lấy từ CCCD #{idx} + Đơn Mẫu 26.\n"
    ...
    return (
        ...
        "NguoiKhai_* (người khai đơn = chủ hồ sơ) trích bình thường từ Đơn Mẫu 26 / Bản khai / CCCD của người "
        "khai. Bỏ trống NguoiNop_*.\n"
        "</nguoi_nop_context>"
    )
