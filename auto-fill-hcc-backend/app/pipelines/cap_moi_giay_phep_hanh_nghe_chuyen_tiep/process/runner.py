"""Compact agent process pipeline cho "Cấp mới giấy phép hành nghề trong giai đoạn chuyển tiếp...".
...
from app.pipelines.cap_moi_giay_phep_hanh_nghe_chuyen_tiep.process.prompt import EXTRA_RULES
from app.pipelines.cap_moi_giay_phep_hanh_nghe_chuyen_tiep.process.schema import (
    ALIASES,
    ALLOWED,
...
async def _submitter_context(documents: list[dict], options: dict) -> str:
    ...
    Đa số tự nộp (người nộp = người hành nghề). Khi NỘP THAY, người nhà có thể upload CẢ CCCD của mình →
    không có mỏ neo, LLM dễ vơ CCCD người nộp làm người hành nghề (chủ hồ sơ). Mỏ neo giúp LLM biết CCCD
    nào là của người nộp để trích NguoiNop_*; người hành nghề lấy từ Đơn Mẫu 08 / Sơ yếu lý lịch Mẫu 09 /
    CCCD của họ.
    """
    ...
    if matched:
        ...
        return (
            ...
            "- NGƯỜI HÀNH NGHỀ (chủ hồ sơ) = người đứng tên trên Đơn Mẫu 08 / Sơ yếu lý lịch Mẫu 09. Trích vào "
            f"các field NguoiHanhNghe_* TỪ các giấy tờ đó, TUYỆT ĐỐI KHÔNG lấy nhân thân người hành nghề từ CCCD "
            f"người nộp (tài liệu #{idx}).\n"
            "- NẾU người nộp CHÍNH LÀ người hành nghề (cùng họ tên hoặc cùng số định danh với Đơn/Mẫu 09) → chỉ "
            f"1 người: BỎ TRỐNG NguoiNop_*, chỉ trích NguoiHanhNghe_* từ CCCD #{idx} + Đơn/Mẫu 09.\n"
    ...
    return (
        ...
        "NguoiHanhNghe_* (người hành nghề = chủ hồ sơ) trích bình thường từ Đơn Mẫu 08 / Sơ yếu lý lịch Mẫu 09 "
        "/ CCCD của người hành nghề. Bỏ trống NguoiNop_*.\n"
        "</nguoi_nop_context>"
    )
