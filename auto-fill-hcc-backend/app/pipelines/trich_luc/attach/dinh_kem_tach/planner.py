"""Phân đoạn tài liệu khi người dùng bật tách hồ sơ trích lục hộ tịch."""
...
def _identity_document_name(subject_name: Any) -> str:
...
async def plan_trich_luc_attachments(
    ...
    for order, segment in enumerate(segments):
        ...
        if doc_type == "blank_page":
            classified.append({
                ...
                "type": doc_type, "documentName": "Trang trắng",
                "target": "ignored", "componentIndex": None,
            })
            continue
        if doc_type == "other":
        ...
        document_name = (
            _unique_document_name(
                _identity_document_name(segment.get("subjectName")), used_names, _IDENTITY_LABEL,
            )
            if doc_type == "identity"
    ...
    if identity_records:
        grouped_identities = merge_identity_records(
            identity_records,
            existing_slot=(3, _ROW_3_COMPONENT),
            default_document_name=_IDENTITY_LABEL,
        )
        used_identity_names: set[str] = set()
        identity_name_by_page: dict[tuple[int, int | None], str] = {}
        for item in grouped_identities:
            subject_name = _clean_subject_name(item.pop("_subjectName", ""))
            base_name = _identity_document_name(subject_name) if subject_name else item.get("documentName")
            ...
            for source in sources:
                ...
                if page_indexes is None:
                    identity_name_by_page[(source["fileIndex"], None)] = document_name
                else:
                    for page_index in page_indexes:
                        identity_name_by_page[(source["fileIndex"], int(page_index) + 1)] = document_name
        for item in classified:
            if item["type"] != "identity":
                continue
            names = {
                identity_name_by_page.get((item["fileIndex"], page))
                or identity_name_by_page.get((item["fileIndex"], None))
                for page in range(item["pageFrom"], item["pageTo"] + 1)
            }
            names.discard(None)
            if len(names) == 1:
                item["documentName"] = next(iter(names))
