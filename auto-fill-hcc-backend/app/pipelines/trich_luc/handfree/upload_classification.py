"""Cấu hình phân loại tệp lúc upload cho thủ tục cấp bản sao trích lục hộ tịch."""
import json
import re
import unicodedata
...
PROCEDURE_KEYS = frozenset({"trich-luc-ks"})
_ALLOWED_KEYS = frozenset({"cccd", "to_khai", "ho_tich", "khac"})

_SYSTEM_PROMPT = """
Bạn là bộ phân loại giấy tờ cho thủ tục cấp bản sao Trích lục hộ tịch, bản sao Giấy khai sinh.
...
- to_khai: Tờ khai cấp bản sao trích lục hộ tịch hoặc Tờ khai cấp bản sao Giấy khai sinh.
- ho_tich: giấy tờ hộ tịch cũ, gồm Giấy khai sinh/trích lục khai sinh, Giấy chứng nhận
  kết hôn/trích lục kết hôn, hoặc trích lục khai tử.
- khac: các giấy tờ liên quan còn lại như văn bản ủy quyền, giấy tờ chứng minh cư trú,
...
nhắc đến giấy khai sinh, kết hôn, khai tử hoặc có số CCCD. Giấy tờ hộ tịch vẫn là
ho_tich dù nội dung có ghi số CCCD của người liên quan. TUYỆT ĐỐI không phân loại
thành cccd chỉ vì tài liệu nhắc đến "CCCD", "căn cước" hoặc có 12 chữ số.
Chỉ dùng OCR_TEXT làm bằng chứng; không dùng tên file hoặc thứ tự file.

Nếu tài liệu đọc rõ nhưng không thuộc cccd, to_khai hoặc ho_tich thì chọn khac.
...
def fallback_to_slot(
    ...
    # Tờ khai xét trước vì phần nội dung thường nhắc chính giấy hộ tịch cần cấp bản sao
    # và ghi cả số CCCD của người yêu cầu.
    if "to khai" in folded and ("cap ban sao" in folded or "trich luc ho tich" in folded):
        if free("to_khai"):
            return "to_khai", None, "fallback theo tiêu đề tờ khai"

    civil_status_markers = (
        "giay khai sinh", "trich luc khai sinh", "giay chung nhan ket hon",
        "trich luc ket hon", "trich luc khai tu",
    )
    if any(marker in folded for marker in civil_status_markers) and free("ho_tich"):
        return "ho_tich", None, "fallback theo giấy tờ hộ tịch cũ"

    # Không tin cụm "CCCD số..." đứng riêng: tờ khai/ủy quyền/giấy hộ tịch đều có thể
    ...
    if free("khac"):
        return "khac", None, "fallback vào Các giấy tờ liên quan khác"
