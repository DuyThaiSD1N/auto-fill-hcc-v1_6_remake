"""Lập kế hoạch đính kèm vào đúng dòng hồ sơ trên cổng DVC Ninh Bình (đính chính GCN đã cấp, Mẫu số 11/ĐK).

componentIndex là vị trí DOM 0-based của dòng (STT − 1); componentName là đoạn text đặc trưng trong cột
"Tên giấy tờ" của dòng đó (khớp substring sau khi fold dấu ở FE). Bố cục 4 dòng lấy VERBATIM từ HTML thật
(html đính kèm.html), KHÔNG lấy từ ảnh minh họa:
- STT1 (index 0): Bản gốc Giấy chứng nhận đã cấp -> land_certificate.
- STT2 (index 1): Giấy tờ chứng minh sai sót thông tin của người được cấp GCN -> error_proof.
- STT3 (index 2): Văn bản về việc ủy quyền theo quy định của pháp luật về dân sự -> authorization.
- STT4 (index 3): Đơn đăng ký biến động đất đai Mẫu số 11/ĐK -> don_bien_dong; CCCD (identity) đi CHUNG
  dòng này vì form KHÔNG có dòng CCCD riêng. Hai docType don_bien_dong/identity cùng componentName+
  componentIndex=3 nên engine attp-row tự gom vào cùng ô upload multiple của dòng 4 — đúng ý đồ.

Phân loại LLM-FIRST tuyệt đối: doc_type CHỈ lấy từ LLM. Không có nhánh rule keyword nào (giòn, dễ sai).
Không rõ/không hợp lệ -> đính CHUNG vào dòng Đơn Mẫu 11 (index 3), KHÔNG bỏ file, kèm cảnh báo.
...
# componentName lấy VERBATIM từ cột "Tên giấy tờ" của html đính kèm.html (đã đối chiếu 4 dòng).
_DON_BIEN_DONG_NAME = "Đơn đăng ký biến động đất đai, tài sản gắn liền với đất theo Mẫu số 11/ĐK"
...
def build_plan_items(
    ...
    for index, file in enumerate(files):
        ...
        # biến động (Mẫu 11/ĐK, index 3). Giữ TÊN FILE GỐC làm documentName để cán bộ nhận ra và soát lại.
        attachments.append({
            "fileIndex": index,
            "fileName": file_name,
        ...
        classified.append({"fileName": file_name, "docType": _OTHER, "source": source, "routedTo": 3})
