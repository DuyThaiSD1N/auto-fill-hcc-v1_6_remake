"""Prompt phân loại đính kèm [Lào Cai — cấp xã] điều chỉnh quyết định giao đất, cho thuê đất (1.115680).

Vẫn phân loại đủ ba loại như bản ở Sở; việc `quyet_dinh_bi_dieu_chinh` và `van_ban_thay_doi_can_cu`
được dồn vào một dòng "Giấy tờ khác" là quyết định của planner, KHÔNG phải của LLM — đừng đổi nhãn
loại để "né" dòng, vì tên dòng gộp được planner đặt cố định theo ánh xạ của cơ quan tiếp nhận.
"""
