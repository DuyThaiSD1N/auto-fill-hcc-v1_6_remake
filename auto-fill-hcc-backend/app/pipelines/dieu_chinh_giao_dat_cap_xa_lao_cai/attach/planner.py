"""Đính kèm [Lào Cai — CẤP XÃ] điều chỉnh quyết định giao đất, cho thuê đất, cho phép chuyển mục đích
sử dụng đất (mã 1.115680, `maCoQuan=UBND_PLC_LCI`).
...
`input[type=file]`** theo thứ tự DOM, mỗi ô kèm nút "Chọn tệp tin" → engine FE `fixed-slot`:
  ...
  0–2 : 3 dòng thành phần hồ sơ (`GiayToCuaHoSoOnline_{50195,50177,50196}_fileGiayTo`)
  3–5 : 3 dòng "Giấy tờ khác" (`HoSoOnline_giayToKhac_file_1..3`)
  6   : ô `HoSoOnline_fileGiayToKhac`

⚠ Ba dòng thành phần trùng TÊN và TRÙNG THỨ TỰ với bản nộp ở Sở (1.115652) nhưng **cách đính khác
hẳn**: ảnh hướng dẫn của cơ quan tiếp nhận yêu cầu ĐỂ TRỐNG dòng 2 và 3, đưa quyết định và văn bản
làm thay đổi căn cứ xuống "Giấy tờ khác" — MỖI TỆP MỘT DÒNG, tên dòng cố định theo loại (xem
`_TEN_DONG_GIAY_TO_KHAC`). `_ROUTES` vẫn giữ đủ ba dòng để đổi lại hành vi chỉ bằng cách rút docType
khỏi `_TEN_DONG_GIAY_TO_KHAC`.
...
# Ánh xạ của cơ quan tiếp nhận (ảnh hướng dẫn kèm hồ sơ mẫu): hai dòng thành phần "giấy chứng nhận/
# quyết định" và "văn bản làm thay đổi căn cứ" ĐỂ TRỐNG; quyết định gốc và văn bản thay đổi căn cứ đi
# xuống "Giấy tờ khác" — cả hai đều là "tài liệu gửi kèm" ở mục 8 của Đơn. Bản nộp ở Sở (1.115652)
# thì ngược lại: route thẳng vào hai dòng đó.
# MỖI TỆP MỘT DÒNG riêng (user chốt): ảnh của BA vẽ chung một dòng, nhưng `ensureOtherListFileRow`
# (content.js) chỉ nhận dòng còn TRỐNG tên nên không dồn được hai tệp vào một dòng mà không phải gộp
# PDF — mà gộp thì cán bộ mất khả năng xem/xoá từng tệp. Tên dòng đặt theo LOẠI để đọc là biết giấy gì.
_TEN_DONG_GIAY_TO_KHAC = {
    "quyet_dinh_bi_dieu_chinh": "bản chính quyết định",
    "van_ban_thay_doi_can_cu": "bản chính văn bản thay đổi căn cứ",
}
...
def build_plan_items(
    ...
    for index, file in enumerate(files):
        ...
        if doc_type == "other" or doc_type in _TEN_DONG_GIAY_TO_KHAC:
            # Hai loại trong `_TEN_DONG_GIAY_TO_KHAC` mang tên dòng CỐ ĐỊNH theo ánh xạ của cơ quan
            # tiếp nhận; loại chưa nhận ra thì lấy tên LLM đọc nội dung đặt, KHÔNG lấy tên tệp (mất
            # dấu, dính số của hệ thống upload).
            component_name = _unique_document_name(
                _TEN_DONG_GIAY_TO_KHAC.get(doc_type) or llm_name, used_names,
            )
            item = {
                "fileIndex": index,
                "fileName": file_name,
            ...
            item = {
                ...
                "slotKey": f"laocai_dcgdx_{doc_type}",
