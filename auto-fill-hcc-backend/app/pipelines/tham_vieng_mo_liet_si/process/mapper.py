"""Map fact Giấy giới thiệu + CCCD + Bằng TQGC sang field Form.io của thủ tục Thăm viếng mộ liệt sĩ.

- Field dùng chung Phần 1 (người nộp) và Phần 4 (chi tiết mẫu khai) điền cùng formcontrolname nhưng
  khác occurrence (0 = Phần 1, 1 = Phần 4): fullname/birthday/gender/identityNumber/identityDate/phoneNumber.
- Phần 4 thường trú người khai = province3/village3/address3 (tên riêng, không occurrence).
- Liệt sĩ quê quán = province4/village4/address4. Bảng người cùng đi = dataGrid2.
- Cư trú ưu tiên TỜ KHAI; số định danh/ngày cấp ưu tiên CCCD.
"""
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    if not candidate_name or not candidate_identity:
        warnings.append("Thiếu họ tên hoặc số định danh người khai từ Giấy giới thiệu/CCCD.")
    ...
    # --- Phần 4: Chi tiết mẫu khai — người khai (occurrence 1, đồng bộ Phần 1) ---
    ...
    add("data[LoaiGiayTo]", _text(values.get("ToKhai_LoaiGiayTo")) or "CCCD")
    add("data[identityAgency]", _issuer(candidate_issue_place) or _text(candidate_issue_place))
    add("data[qheLSi]", _text(values.get("ToKhai_QuanHeVoiLietSi")))
    # Thường trú người khai → province3/village3/address3.
    _add_area(add, "data[province3]", "data[village3]", "data[address3]", candidate_residence)

    # --- Thông tin liệt sĩ ---
    add("data[fullname1]", _text(values.get("LietSi_HoTen")))
    _add_area(add, "data[province4]", "data[village4]", "data[address4]", values.get("LietSi_QueQuan"))
    add("data[Coquandonvihisinh]", _text(values.get("LietSi_CoQuanDonVi")))
    add("data[CapBacChucVuKhiHiSinh]", _text(values.get("LietSi_CapBac")))
    add("data[ngayHiSinh]", _date(values.get("LietSi_NgayHySinh")))
    add("data[noiHiSinh]", _text(values.get("LietSi_NoiHySinh")))
    add("data[soBang]", _text(values.get("LietSi_SoBang")))

    # --- Bảng người cùng đi (dataGrid2) ---
    nguoi_di = _list(values.get("ToKhai_NguoiCungDi"))
    for idx, item in enumerate(nguoi_di[:8]):
        add(f"data[dataGrid2][{idx}][textField2]", _item_text(item, "hoTen", "hoVaTen"))
        add(f"data[dataGrid2][{idx}][NgayThangNamSinh]", _date(_item_text(item, "ngaySinh", "namSinh")))
        add(f"data[dataGrid2][{idx}][SoCCCMND]", _identity(_item_text(item, "soGiayTo", "soCccd", "soCmnd")))
        add(f"data[dataGrid2][{idx}][asd]", _date(_item_text(item, "ngayCap")) or _item_text(item, "ngayCap"))
        add(f"data[dataGrid2][{idx}][asd1]", _issuer(_item_text(item, "noiCap")) or _item_text(item, "noiCap"))
        add(f"data[dataGrid2][{idx}][QuanHeVoiLietSiDiCung]", _item_text(item, "moiQuanHe", "mqh", "quanHe"))

    # --- Khai báo hồ sơ đính kèm (dòng đầu) ---
    add("data[hoSoDinhKem][0][textField1]", "Giấy giới thiệu thăm viếng mộ liệt sĩ")
    add("data[hoSoDinhKem][0][textField2]", "Bản chính")

    if not values.get("LietSi_HoTen"):
        warnings.append("Chưa đọc được họ tên liệt sĩ cần thăm viếng trong Giấy giới thiệu.")
    return out, warnings
