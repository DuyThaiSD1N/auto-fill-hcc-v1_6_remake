"""Đính kèm [Lào Cai] 1.115677 — bảng "Thành phần hồ sơ" bước 3 của cổng `dichvucong.laocai.gov.vn`.

Bảng PHẲNG, đúng 2 dòng theo thứ tự DOM; mỗi dòng có checkbox chọn giấy tờ + ô "Số bản" + nút "Chọn
tệp tin" (sau khi tích còn hiện thêm cột "Loại" mặc định "Bản"):

  0. Văn bản đề nghị xác nhận lại thời hạn sử dụng đất theo Mẫu số 39 ban hành kèm theo Quyết định số
     47/2026/QĐ-UBND
  1. Giấy chứng nhận đã cấp...

Bên dưới là khối "Thông tin khác" / "Thành phần hồ sơ khác nếu có" → danh sách "Giấy tờ khác" (3 dòng
mặc định: chọn Loại "Mới", tự gõ tên giấy tờ rồi chọn tệp).
...
`giao_thue_dat_lao_cai` và `dk_giay_cn_thua_dat_dien_tich_tang_them`) để FE dùng thẳng thứ tự DOM — vừa
khỏi phải sửa extension, vừa tránh nhầm giữa hai dòng vốn cùng nói về "sử dụng đất".

`tickRow: True` vì cột "#" của mỗi dòng là checkbox chọn giấy tờ — không tích thì cổng không nhận tệp.

MỘT TỆP chỉ đi ĐÚNG MỘT DÒNG: không đính một tệp vào hai dòng (số lượt đính > số tệp làm extension báo
"3/2 file"), cũng KHÔNG cắt trang bằng sourceSegments — cắt sai là mất giấy tờ.

Hệ quả đã biết của quy tắc trên (đúng như ảnh ánh xạ hồ sơ mẫu): file Giấy chứng nhận quét gộp luôn cả
MẢNH TRÍCH ĐO ở trang cuối, nên mảnh trích đo nằm cùng dòng 1 chứ không tách xuống "Giấy tờ khác" —
planner phát cảnh báo để cán bộ tự quyết.

CCCD/giấy tờ nhân thân KHÔNG thuộc thành phần hồ sơ của thủ tục này; hồ sơ mẫu không nộp kèm. Nhưng nếu
người dân có upload thì vẫn đưa xuống "Giấy tờ khác" (kèm cảnh báo) thay vì âm thầm bỏ tệp.
...
# 2 dòng cố định của bảng; `slotIndex` là thứ tự DOM 0-based của các ô upload.
_ROWS: dict[int, str] = {
    0: "Văn bản đề nghị xác nhận lại thời hạn sử dụng đất theo Mẫu số 39 ban hành kèm theo Quyết định "
       "số 47/2026/QĐ-UBND",
    1: "Giấy chứng nhận đã cấp",
}
...
_ROUTES: dict[str, tuple[int, str]] = {
    "don_mau_39": (0, "Đơn đề nghị xác nhận lại thời hạn sử dụng đất nông nghiệp (Mẫu số 39)"),
...
_OTHER_ROUTES: dict[str, str] = {
    ...
    "van_ban_dai_dien": "Văn bản về việc đại diện (Giấy ủy quyền)",
...
# Cả HAI dòng đều là thành phần hồ sơ BẮT BUỘC của thủ tục — trống thì phải nhắc, không im lặng.
_ROWS_CAN_NHAC = {
    0: "Văn bản đề nghị xác nhận lại thời hạn sử dụng đất (Mẫu số 39)",
    1: "Giấy chứng nhận đã cấp",
}
...
_HAY_QUET_GOP: dict[str, tuple[str, str]] = {
    "manh_trich_do": ("gcn", "mảnh trích đo bản đồ địa chính thửa đất"),
}
...
def build_plan_items(
    ...
    for index, file in enumerate(files):
        ...
        if doc_type in _ROUTES:
            ...
            attachments.append({
                ...
                # slotKey theo DÒNG (không theo docType) để nhiều tệp cùng dòng được engine GOM vào một
                # lần bơm tệp, thay vì lần sau thấy ô đã dùng rồi báo lỗi.
                "slotKey": f"lc_115677_row_{slot_index}",
    ...
    if thieu:
        warnings.append(
            "Chưa có tệp nào cho dòng: " + "; ".join(thieu) + ". Đây là thành phần hồ sơ bắt buộc của "
            "thủ tục — cán bộ đề nghị người dân bổ sung rồi tích dòng."
        )
    ...
    for doc_type, (host_type, mo_ta) in _HAY_QUET_GOP.items():
        if doc_type not in have_types and host_type in have_types:
            ...
            warnings.append(
                f"Không thấy tệp riêng cho {mo_ta}. Hồ sơ dạng này thường quét gộp nó vào trang cuối "
                f"file \"{_ROUTES[host_type][1]}\" (dòng \"{host_row[:60]}…\"). Mỗi tệp chỉ được xếp vào "
                "MỘT dòng nên nó đi kèm ở đó; nếu nơi tiếp nhận yêu cầu tách riêng thì cán bộ đề nghị "
                "người dân tách tệp rồi đính bổ sung vào \"Giấy tờ khác\"."
            )

    # Giấy tờ nhân thân không nằm trong thành phần hồ sơ của thủ tục — nói rõ để cán bộ khỏi tưởng lọt.
    if "giay_to_nhan_than" in have_types:
        warnings.append(
            "CCCD/giấy tờ hộ tịch KHÔNG thuộc thành phần hồ sơ của thủ tục này; trợ lý đưa xuống \"Giấy "
            "tờ khác\" để không bỏ sót tệp. Nơi tiếp nhận không yêu cầu thì cán bộ xoá dòng đó đi."
        )
