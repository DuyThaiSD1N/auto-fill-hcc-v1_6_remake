"""Đính kèm cho [Lào Cai] Chuyển mục đích sử dụng đất cấp phường/xã (1.115679).

LLM đọc OCR từng file → nhãn rút gọn (catalog.py). BE đổi nhãn thành:
  - ô fixed-slot của đúng MỘT dòng bảng "Thành phần hồ sơ" (FE khoanh vùng theo sectionHeader, tích checkbox
    dòng rồi bơm tệp);
  - hoặc một dòng "Giấy tờ khác" (target=new, tên = documentName) cho giấy tờ không có dòng riêng: giấy uỷ
    quyền, hồ sơ nghĩa vụ tài chính, hồ sơ đo đạc, GCN đăng ký doanh nghiệp, thông báo kết quả, tài liệu khác.
CCCD không phải thành phần hồ sơ → bỏ qua kèm cảnh báo.

MỖI TỆP CHỈ VÀO MỘT DÒNG: hồ sơ mẫu của thủ tục này quét gộp cả cụm nghĩa vụ tài chính + đo đạc vào một PDF
(14 trang, 6,33 MB) — vượt trần 6 MB của cổng nên planner loại từ đầu và báo lại để cán bộ tách tệp.
"""
...
# Cổng Lào Cai: "Đính kèm Tệp tin vào các thành phần hồ sơ tương ứng… Dung lượng tối đa là 6 Mb".
...
def build_plan_items(
    ...
    if not labels.intersection(catalog.DON_LABELS):
        warnings.append(
            "Không thấy Đơn đề nghị (Mẫu số 02 chuyển mục đích, Mẫu số 03 chuyển hình thức hoặc Mẫu số 17 "
            "gia hạn) — đây là thành phần bắt buộc, dòng Đơn sẽ để trống. Cán bộ kiểm tra lại hồ sơ."
        )
    if "don_mau_02" in labels and "gcn" not in labels:
        warnings.append(
            "Hồ sơ chuyển mục đích nhưng không thấy Giấy chứng nhận quyền sử dụng đất — dòng "
            f"\"{catalog.row_name(catalog.ROW_GCN)}\" sẽ để trống. Nếu Giấy chứng nhận nằm chung tệp với "
            "giấy tờ khác thì cán bộ tách riêng rồi đính bổ sung."
        )
    if unknown:
        warnings.append(
...
async def plan(files: list[FileItem], options: dict | None = None, session: dict | None = None) -> dict:
    ...
    for file in raw_files:
        ...
        if size > _MAX_FILE_BYTES:
            errors.append(
                f"File '{file['name']}' vượt quá 6 MB ({size / 1024 / 1024:.2f} MB) — cổng Lào Cai không "
                "nhận, đã bỏ qua. Hãy tách tệp hoặc quét lại ở DPI thấp hơn rồi đính bổ sung."
            )
        else:
