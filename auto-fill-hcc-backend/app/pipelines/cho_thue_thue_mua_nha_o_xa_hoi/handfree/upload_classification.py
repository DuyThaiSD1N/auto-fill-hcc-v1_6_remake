"""Phân loại tệp lúc upload (checklist handfree) cho thủ tục Cho thuê, cho thuê mua nhà ở xã hội.

Chỉ là lớp xếp tệp vào checklist khi công dân tải ảnh. Phân loại thành phần hồ sơ trên cổng
Bộ Xây dựng vẫn do ``attach/planner.py`` quyết định từ toàn bộ OCR (4 dòng attp-row).
"""
import json
import re
import unicodedata
...
PROCEDURE_KEYS = frozenset({"cho-thue-thue-mua-nha-o-xa-hoi"})
_ALLOWED_KEYS = frozenset({"don", "cccd", "doi_tuong", "dieu_kien", "khac"})

_SYSTEM_PROMPT = """
Bạn là bộ phân loại giấy tờ cho thủ tục Cho thuê, cho thuê mua nhà ở xã hội do Nhà nước
...
- don: TỜ ĐƠN đăng ký thuê / thuê mua nhà ở xã hội theo mẫu. Tiêu đề có "ĐƠN ĐĂNG KÝ
  THUÊ" hoặc "THUÊ MUA NHÀ Ở XÃ HỘI", nội dung khai nhân thân, thực trạng nhà ở, thành
...
- doi_tuong: giấy tờ chứng minh ĐỐI TƯỢNG chính sách — Huân/Huy chương kháng chiến, bằng
  khen, Giấy chứng nhận thương binh/bệnh binh, Giấy báo tử/chứng nhận thân nhân liệt sĩ,
  giấy tờ quân nhân/công an, quyết định miễn giảm tiền thuê nhà.
- dieu_kien: giấy tờ chứng minh ĐIỀU KIỆN nhà ở / thu nhập — xác nhận hộ nghèo/cận nghèo,
  xác nhận thu nhập, xác nhận thực trạng nhà ở, xác nhận chưa có nhà ở.
- khac: giấy tờ liên quan còn lại.
- unknown: OCR trống hoặc quá thiếu để xác định an toàn.

Phân loại theo TIÊU ĐỀ và BẢN CHẤT của tài liệu. Tờ đơn vẫn là don dù bên trong ghi số căn
cước hoặc nhắc đối tượng chính sách. TUYỆT ĐỐI không phân loại thành cccd chỉ vì tài liệu
...
def fallback_to_slot(
    ...
    # Tờ đơn xét trước: đơn nhắc cả đối tượng chính sách lẫn số căn cước bên trong.
    if ("dang ky thue" in folded or "thue mua nha o xa hoi" in folded) and free("don"):
        return "don", None, "fallback theo tiêu đề tờ đơn"

    if any(marker in folded for marker in (
        "huan chuong", "huy chuong", "thuong binh", "benh binh", "liet si",
        "quan nhan", "mien giam tien thue",
    )) and free("doi_tuong"):
        return "doi_tuong", None, "fallback theo giấy chứng minh đối tượng"

    if any(marker in folded for marker in (
        "ho ngheo", "can ngheo", "xac nhan thu nhap", "thuc trang nha o", "chua co nha o",
    )) and free("dieu_kien"):
        return "dieu_kien", None, "fallback theo giấy chứng minh điều kiện"
