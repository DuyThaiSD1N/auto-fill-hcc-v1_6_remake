"""Map facts nguồn sang Form.io của cổng DVC Ninh Bình (đăng ký đất đai, cấp GCN lần đầu)."""
...
# Form không có dòng đính kèm riêng cho CCCD nên CCCD của (các) người liên quan đi chung file giấy tờ
# nhận thừa kế; ghi chú cố định này giúp cán bộ tiếp nhận biết vị trí CCCD trong hồ sơ.
_NOTE_TEXT = (
    "CCCD/Căn cước của (các) người liên quan được đính kèm chung trong file giấy tờ nhận thừa kế "
    "(không có dòng riêng cho CCCD)."
)
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    # Đặc thù form: luôn điền ghi chú CCCD đi chung file thừa kế (không có dòng CCCD riêng).
    add("data[note]", _NOTE_TEXT, default=True)
