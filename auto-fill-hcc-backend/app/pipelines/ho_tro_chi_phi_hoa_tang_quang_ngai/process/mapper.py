"""Map facts nguồn sang Form.io của cổng DVC Quảng Ngãi (hỗ trợ chi phí hỏa táng)."""
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    # Tên chủ hồ sơ khác tên người nộp => hồ sơ có ủy quyền/giấy giới thiệu (cổng Quảng Ngãi không có
    ...
    # Template dùng chung của cổng Quảng Ngãi KHÔNG có khối riêng cho người nộp. Panel "Thông tin
    # chung" (ngày sinh, giới tính, email, số điện thoại, số định danh, ngày/nơi cấp, địa chỉ) đều là
    # thông tin CHỦ HỒ SƠ; về người nộp chỉ có đúng 2 ô: "Họ và tên người nộp hồ sơ" (data[fullname])
    # và "Số điện thoại ủy quyền" (data[phoneNumber1]). Vì vậy KHÔNG được đổi nguồn khối này sang
    # NguoiNop_* khi có ủy quyền — làm thế là ghi nhân thân người được ủy quyền vào ô của chủ hồ sơ.
    ...
    # Đối tượng đề nghị quyết định dùng Tờ khai Mẫu số 01 (cá nhân) hay Mẫu số 02 (cơ quan, tổ chức).
    # Cá nhân là mặc định của cổng và là trường hợp phổ biến -> phát kèm default=True để extension tô
    # viền vàng, cán bộ tự đổi khi hồ sơ là cơ quan, tổ chức.
    ...
    # nation2): panel này thuộc template đất đai/xây dựng dùng chung của cổng, thủ tục hỏa táng không
    # phát sinh thửa đất. 4 ô đó cũng không được khai trong UI_COMP_BY_NAME nên không có đường nào
    # nhét nơi cư trú hay địa chỉ cơ sở hỏa táng vào.
