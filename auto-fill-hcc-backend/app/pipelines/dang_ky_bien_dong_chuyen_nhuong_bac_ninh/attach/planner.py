"""Đính kèm "[Bắc Ninh] Đăng ký biến động QSDĐ (chuyển nhượng/thừa kế/tặng cho/góp vốn…)".
...
thành phần theo mã KQ (in trong dòng tiêu đề mỗi thành phần — bền hơn mô tả dài dễ trùng).
"""
import re
import time
...
def _label_by_keywords(ocr_text: str) -> str | None:
    ...
    if not h.strip():
        return None
    # GCN THẬT (sổ đỏ) có "số vào sổ" — dấu hiệu chỉ có trên Giấy chứng nhận. Ưu tiên cả khi file gộp.
    if "so vao so" in h and "giay chung nhan" in h:
        return "gcn"
    # Đơn đăng ký biến động nhận diện TRƯỚC "giấy chứng nhận" (thân đơn hay nhắc GCN → dễ nhầm).
    if "don dang ky bien dong" in h or ("dang ky bien dong" in h and "don" in h):
        return "don_bien_dong"
    if "hop dong chuyen nhuong" in h or "hop dong tang cho" in h or "hop dong gop von" in h:
        return "hop_dong_chuyen_quyen"
    if ("khai nhan di san" in h or "phan chia di san" in h or "van ban thua ke" in h):
        return "hop_dong_chuyen_quyen"
    if "bien ban hop" in h and "tang cho" in h:
        return "bien_ban_hop_xa_tang_cho"
    if "ban ve tach thua" in h or "hop thua" in h or "mau so 22" in h:
        return "ban_ve_tach_thua"
    if "trich do" in h and ("dia chinh" in h or "thua dat" in h):
        return "manh_trich_do"
    if "van ban thoa thuan" in h and "chung mot giay chung nhan" in h:
        return "vb_thoa_thuan_chung_gcn"
...
def _label_by_filename(name: str) -> str | None:
    ...
    if "can cuoc" in h or "cccd" in h or "cmnd" in h or "cmt" in h:
        return "cccd"
    if "hop dong" in h or "chuyen nhuong" in h or "tang cho" in h or "thua ke" in h or "gop von" in h:
        return "hop_dong_chuyen_quyen"
    if "don" in h and "bien dong" in h:
        return "don_bien_dong"
    if any(k in h for k in ("so dat", "so do", "so hong", "gcn", "qsdd", "chung nhan")):
        return "gcn"
    if "tach thua" in h or "mau 22" in h:
        return "ban_ve_tach_thua"
    if "trich do" in h:
        return "manh_trich_do"
    return None
