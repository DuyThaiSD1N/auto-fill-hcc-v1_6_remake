"""Ánh xạ facts → ô eForm của thủ tục [Lào Cai] đính chính Giấy chứng nhận đã cấp lần đầu có sai sót.
...
⚑ Ô "Về việc": GIỮ CÂU MẶC ĐỊNH của cổng rồi NỐI THÊM nội dung đính chính cụ thể (sai sót gì, trên
Giấy chứng nhận số nào). Ô "Ghi chú": mô tả tệp Giấy chứng nhận và người đồng sử dụng đất.
"""
...
from app.pipelines.dinh_chinh_gcn_da_cap_lao_cai.process.schema import (
    UI_ALIASES,
...
_VE_VIEC_MAC_DINH = "Đính chính Giấy chứng nhận đã cấp lần đầu có sai sót"


def _gcn_mo_ta(values: dict) -> str | None:
    """Cụm định danh Giấy chứng nhận: số phát hành + số vào sổ + ngày cấp (chỉ phần đọc được)."""
    parts = []
    so_phat_hanh = _plain(values.get("Gcn_SoPhatHanh"))
    if so_phat_hanh:
        parts.append(f"số phát hành {so_phat_hanh}")
    so_vao_so = _plain(values.get("Gcn_SoVaoSo"))
    if so_vao_so:
        parts.append(f"số vào sổ cấp GCN {so_vao_so}")
    ngay_cap = _date(values.get("Gcn_NgayCap"))
    if ngay_cap:
        parts.append(f"cấp ngày {ngay_cap}")
    return ", ".join(parts) or None


def _ve_viec(values: dict) -> str | None:
    """Ô "Về việc" — cổng điền sẵn TÊN THỦ TỤC, nghiệp vụ yêu cầu giữ câu đó rồi bổ sung nội dung cụ thể.

    Không đọc được nội dung đính chính thì KHÔNG phát field: ghi mỗi tên thủ tục là đúng bằng việc để
    nguyên ô cổng đã điền.
    """
    noi_dung = _plain(values.get("Don_NoiDungDinhChinh"))
    if not noi_dung:
        return None
    cau = f"{_VE_VIEC_MAC_DINH} – {noi_dung[0].lower() + noi_dung[1:]}"
    gcn = _gcn_mo_ta(values)
    if gcn:
        # Người dân hay viết sẵn "… trên giấy chứng nhận quyền sử dụng đất" trong đơn; nối thêm một
        # cụm "trên Giấy chứng nhận …" nữa là câu lặp, nên khi đó chỉ mở ngoặc bổ sung số hiệu.
        cau += f" ({gcn})" if "giay chung nhan" in _fold(noi_dung) else f" trên Giấy chứng nhận {gcn}"
    return cau.rstrip(" .") + "."


def _ghi_chu(values: dict) -> str | None:
    """Ô "Ghi chú" — mô tả thửa đất trên Giấy chứng nhận và người đồng sử dụng đất.

    Người đồng sử dụng là thứ cán bộ phải biết: Đơn thường chỉ có chữ ký của MỘT người, người còn lại
    phải ký bổ sung hoặc có văn bản ủy quyền.

    Danh mục tệp KHÔNG ghép ở đây: bước process chưa biết kết quả phân loại tệp — planner phát cảnh
    báo để cán bộ bổ sung (nhất là khi một tệp gộp nhiều Giấy chứng nhận).
    """
    phan = []
    ...
    if so_thua:
        thua.append(f"Thửa đất số {so_thua}")
    to_ban_do = _plain(values.get("ThuaDat_ToBanDo"))
    if to_ban_do:
        thua.append(f"tờ bản đồ {to_ban_do}")
    dien_tich = _plain(values.get("ThuaDat_DienTich"))
    if dien_tich:
        thua.append(f"diện tích {dien_tich}")
    ...
    if thua_dat:
        ...
        if dia_chi:
            thua.append(f"tại {dia_chi}")
    if thua:
        phan.append(", ".join(thua))

    dong_su_dung = _plain(values.get("DongSuDung_HoTen"))
    if dong_su_dung:
        cau = f"Người đồng sử dụng đất: {dong_su_dung}"
        so_dinh_danh = _digits(values.get("DongSuDung_SoDinhDanh"))
        if so_dinh_danh:
            cau += f", CCCD {so_dinh_danh}"
        phan.append(cau)
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    add("HoSoOnline_veViec", _ve_viec(values))
