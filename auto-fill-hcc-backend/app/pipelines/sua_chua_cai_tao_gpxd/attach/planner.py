"""Đính kèm "Thành phần hồ sơ" cho "Cấp GPXD sửa chữa, cải tạo" (cổng Bộ Xây dựng dvc.moc.gov.vn —
...
rule keyword chỉ DỰ PHÒNG. CCCD/ủy quyền/chứng chỉ → other (bỏ qua).
"""
...
_DON = "don_gpxd"
_GCN = "gcn_qsdd"
_BV_HIEN_TRANG = "ban_ve_hien_trang"
_BV_THIET_KE = "ban_ve_thiet_ke"
_ANH = "anh_hien_trang"
_OTHER = "other"
...
_ROWS: dict[str, dict[str, str]] = {
    _DON: {
        "componentName": "Đơn đề nghị cấp giấy phép xây dựng theo Mẫu số 01",
        "loaiBan": "Bản chính",
        "documentName": "Đơn đề nghị cấp GPXD (Mẫu số 01)",
    },
    _GCN: {
        "componentName": "giấy tờ hợp pháp về đất đai chứng minh sự phù hợp mục đích sử dụng đất",
        "loaiBan": "Bản chính",
        "documentName": "Giấy tờ hợp pháp về đất đai (GCN QSDĐ)",
    },
    _BV_HIEN_TRANG: {
        "componentName": "Bản vẽ hiện trạng của các bộ phận công trình dự kiến sửa chữa",
        "loaiBan": "Bản chính",
        "documentName": "Bản vẽ hiện trạng bộ phận công trình sửa chữa, cải tạo",
    },
    _BV_THIET_KE: {
        "componentName": "Bộ bản vẽ thiết kế xây dựng trong hồ sơ thiết kế xây dựng",
        "loaiBan": "Bản chính",
        "documentName": "Bộ bản vẽ thiết kế xây dựng (phương án sửa chữa, cải tạo)",
    },
    _ANH: {
        "componentName": "hiện trạng công trình và công trình lân cận trước khi sửa chữa",
        "loaiBan": "Bản chính",
        "documentName": "Ảnh chụp hiện trạng công trình và công trình lân cận",
    },
}
...
def _rule_doc_type(text: str) -> str:
    ...
    if not h:
        return ""
    if "don de nghi cap giay phep xay dung" in h or ("don de nghi" in h and "giay phep xay dung" in h and "mau so 01" in h):
        return _DON
    if "giay chung nhan quyen su dung dat" in h or ("giay chung nhan" in h and "quyen su dung dat" in h):
        return _GCN
    if "ban ve hien trang" in h:
        return _BV_HIEN_TRANG
    if ("ban ve thiet ke" in h or "thiet ke ky thuat" in h or "thiet ke ban ve thi cong" in h) and "hien trang" not in h:
        return _BV_THIET_KE
    if ("anh chup" in h or "hinh anh" in h) and "hien trang" in h:
        return _ANH
    return ""
...
def _normalize_doc_type(value: str) -> str:
    ...
    if text in _ALLOWED_DOC_TYPES:
        return text
    if "don" in text and "giay phep" in text:
        return _DON
    if "hien trang" in text and ("anh" in text or "hinh" in text):
        return _ANH
    if "ban ve" in text and "hien trang" in text:
        return _BV_HIEN_TRANG
    if "ban ve" in text or "thiet ke" in text:
        return _BV_THIET_KE
    if "dat" in text or "qsdd" in text or "chung nhan" in text or "so do" in text:
        return _GCN
    return _OTHER
