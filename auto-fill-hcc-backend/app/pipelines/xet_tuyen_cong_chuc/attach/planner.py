"""Đính kèm cho thủ tục Xét tuyển công chức.
...
from app.pipelines.xet_tuyen_cong_chuc.attach import prompt
...
SLOT = {
    ...
    "slotIndex": 0,
    "slotName": "Phiếu đăng ký dự tuyển theo Mẫu số 01",
...
def detect_slot_key(text: str, file_name: str = "") -> str:
    ...
    if not haystack:
        return ""
    if "phieu dang ky du tuyen" in haystack and (
        "cong chuc" in haystack or "nghi dinh so 170/2025" in haystack or "vi tri viec lam du tuyen" in haystack
    ):
        return SLOT["slotKey"]
    if "mau so 01" in haystack and "vi tri viec lam du tuyen" in haystack and "co quan to chuc don vi du tuyen" in haystack:
        return SLOT["slotKey"]
...
async def plan(
    ...
    """Entry point đính kèm cho xet-tuyen-cong-chuc."""
    _ = session
    _ = options or {}
    errors: list[str] = []
