"""Đính kèm cho thủ tục Đăng ký thành lập công ty TNHH hai thành viên trở lên.

Cùng khuôn với app/pipelines/thanh_lap_ctcp/attach: OCR + LLM CHỈ để phân loại tài liệu và đặt tên
hiển thị; việc route sang category của cổng là TẤT ĐỊNH (đọc marker trong OCR trước, LLM chỉ là lưới
đỡ). Extension nhận `attachments` rồi tự khai loại + tải file lên cổng.

`category` ở đây là hợp đồng NGHIỆP VỤ (mã nội bộ), còn thứ extension dùng để khớp option trên cổng
là NHÃN tiếng Việt ở `componentName`/`detectedType`. Mã option thật của cổng chỉ đọc được từ DOM,
chưa có nên KHÔNG bịa.

Khác bản công ty cổ phần: không có "Danh sách cổ đông sáng lập"; thay bằng "Danh sách thành viên" và
"Danh sách chủ sở hữu hưởng lợi của doanh nghiệp" (thành phần hồ sơ theo Nghị định 168/2025/NĐ-CP).
"""
...
_CAT_MEMBERS = "MEMBERLIST"      # Danh sách thành viên
_CAT_BENEFICIAL = "BOLIST"       # Danh sách chủ sở hữu hưởng lợi của doanh nghiệp
...
_LABEL_BY_CAT = {
    ...
    _CAT_MEMBERS: "Danh sách thành viên",
    _CAT_BENEFICIAL: "Danh sách chủ sở hữu hưởng lợi của doanh nghiệp",
...
_LLM_TO_CAT = {
    ...
    "member_list": _CAT_MEMBERS,
    "beneficial_owner_list": _CAT_BENEFICIAL,
...
_BENEFICIAL_MARKERS = (
    "danh sach chu so huu huong loi",
    "chu so huu huong loi cua doanh nghiep",
...
def _detect_category(ocr_text: str) -> str | None:
    ...
    if not haystack.strip():
        return None
    # Giấy đề nghị xét TRƯỚC: nó cũng chứa bảng nguồn vốn và số định danh cá nhân, dễ bị bắt nhầm
    # sang danh sách thành viên hoặc giấy tờ cá nhân.
    if any(m in haystack for m in _BUSREG_MARKERS):
        return _CAT_BUSREG
    # Điều lệ cũng liệt kê thành viên -> phải xét trước danh sách; dấu hiệu chắc là "chương"/"điều".
    if "dieu le" in haystack and ("chuong" in haystack or "dieu 1" in haystack):
        return _CAT_CHARTER
    # Chủ sở hữu hưởng lợi xét TRƯỚC danh sách thành viên: hai bảng gần như trùng cột, nhưng giấy chủ
    # sở hữu hưởng lợi luôn có riêng cụm "chủ sở hữu hưởng lợi" ở tiêu đề.
    if any(m in haystack for m in _BENEFICIAL_MARKERS):
        return _CAT_BENEFICIAL
    if any(m in haystack for m in _MEMBER_MARKERS):
        return _CAT_MEMBERS
