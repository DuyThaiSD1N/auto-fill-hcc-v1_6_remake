"""Map compact source facts to DOM UI fields for water-supply installation."""

import re
from typing import Any

from app.pipelines._shared.compact_agent.issuer import default_issuer
from app.pipelines.cap_nuoc_sach.process.schema import UI_ALIASES, UI_COMP_BY_NAME
...
def enrich(fields: list[dict]) -> list[dict]:
    ...
    thuong_tru = values.get("Cccd_ThuongTru")
    ...
    add("CongDan_tenCoQuanToChuc", values.get("Don_TenCoQuanToChuc"))
    add("CongDan_maSoThueNguoiNop", _phone(values.get("Don_MaSoThue")) or _text(values.get("Don_MaSoThue")))
    ...
    add("CongDan_maTinhThanh", _field(que_quan, "tinh", "tinhThanh"))
    add("CongDan_maPhuongXa", _field(que_quan, "xa", "phuongXa"))
    add("CongDan_diaChi", _field(que_quan, "diaChi", "chiTiet", "thonXom", "soNha"))
    add("CongDan_diDong", _phone(values.get("Don_SoDienThoai")))
    ...
    add("CongDan_noiOHienTai", _text(values.get("Don_DiaChiDeNghiCapNuoc")) or _full_address(thuong_tru))
    add("CongDan_diaChiThuongTru", _full_address(thuong_tru))
