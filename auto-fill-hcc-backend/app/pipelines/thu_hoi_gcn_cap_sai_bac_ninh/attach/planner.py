"""Đính kèm cho "[Bắc Ninh] Thu hồi GCN cấp sai & cấp lại".

LLM phân loại là chính → NHÃN RÚT GỌN (catalog) → componentName (mã KQ) hoặc ô bổ sung.
Rule từ khóa/tên file chỉ fallback khi OCR rỗng/LLM không chắc.
"""
import re
import time
...
def _looks_like_identity(h: str) -> bool:
    if any(k in h for k in ("can cuoc cong dan", "the can cuoc", "cccd", "chung minh nhan dan", "ho chieu", "passport")):
        return True
...
def _label_by_keywords(ocr_text: str) -> str | None:
    ...
    if not h.strip():
        return None
    if "bien ban" in h or "kien nghi" in h or ("de nghi" in h and "thu hoi" in h):
        return "van_ban_kien_nghi"
...
def _label_by_filename(name: str) -> str | None:
    ...
    if not h.strip():
        return None
    if "bien ban" in h or "hop gd" in h or "hop gia dinh" in h or "kien nghi" in h:
        return "van_ban_kien_nghi"
    if "can cuoc" in h or "cccd" in h or "cmnd" in h or "cmt" in h:
        return "cccd"
    if any(k in h for k in ("so dat", "so do", "so hong", "gcn", "qsdd", "chung nhan", "so 1", "so.")):
        return "gcn"
    return None
