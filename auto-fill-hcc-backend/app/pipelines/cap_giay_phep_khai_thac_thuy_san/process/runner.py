"""Compact agent process pipeline cho "Cấp, cấp lại Giấy phép khai thác thủy sản".

HAI vai (thường trùng): NGƯỜI ĐỀ NGHỊ (chủ tàu = chủ hồ sơ) và NGƯỜI NỘP. context_builder neo NGƯỜI NỘP
(tên + CCCD tài khoản cổng tự đổ vào Phần I) để LLM tách khỏi chủ tàu khi có nộp thay."""

import re
import unicodedata

from app.pipelines._shared.compact_agent import runner
from app.pipelines.cap_giay_phep_khai_thac_thuy_san.process import mapper
from app.pipelines.cap_giay_phep_khai_thac_thuy_san.process.prompt import EXTRA_RULES
from app.pipelines.cap_giay_phep_khai_thac_thuy_san.process.schema import (
    ALIASES,
    ALLOWED,
...
async def _submitter_context(documents: list[dict], options: dict) -> str:
    ...
    if matched:
        ...
        return (
            ...
            f"tài liệu #{idx}.\n"
            "- NGƯỜI ĐỀ NGHỊ (chủ tàu, chủ hồ sơ) = người đứng tên trên Đơn Mẫu 04/05.KT. Trích vào các "
            f"field NguoiDeNghi_* TỪ Đơn + CCCD của họ, TUYỆT ĐỐI KHÔNG lấy nhân thân chủ tàu từ CCCD người "
            f"nộp (tài liệu #{idx}).\n"
            "- NẾU người nộp CHÍNH LÀ chủ tàu (cùng họ tên hoặc cùng số định danh với Đơn) → chỉ 1 người: "
            f"BỎ TRỐNG NguoiNop_*, chỉ trích NguoiDeNghi_* từ CCCD #{idx} + Đơn.\n"
    ...
    return (
        ...
        "NguoiDeNghi_* trích bình thường từ Đơn Mẫu 04/05.KT / CCCD của chủ tàu. Bỏ trống NguoiNop_*.\n"
        "</nguoi_nop_context>"
    )
