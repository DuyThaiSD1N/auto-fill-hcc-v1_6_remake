"""Bù các nhãn tất định mà LLM đôi khi bỏ sót trong hồ sơ cấp lại/cấp đổi HKD."""
...
def _address(text: str) -> dict[str, str] | None:
    ...
    if len(lines) > 1 and len(region) >= 2:
        detail, commune, province = ", ".join(lines[:-1]), region[0], region[-1]
    else:
    ...
    return {"quocGia": "Việt Nam", "tinh": province, "xa": commune, "diaChi": detail}
...
def apply_ocr_fallback(raw_fields: Any, documents: list[dict]) -> dict[str, Any]:
    """Chỉ bù mã có nhãn rõ và thẻ vật lý; không tự gán thẻ thành người ký."""
