"""Phân loại tài liệu vào danh mục đính kèm của hồ sơ cấp lại/cấp đổi GCN HKD."""
...
_TYPE_CONFIG = {
    "reissue_application": ("BUSREISSUEFRM", "Giấy đề nghị cấp lại Giấy chứng nhận đăng ký hộ kinh doanh"),
    "other": ("OTHERS", "Khác"),
}
_MARKERS = (
...
def _detect_type(text: str) -> str | None:
    ...
    return "reissue_application" if any(marker in folded for marker in _MARKERS) else None
...
async def _classify(documents: list[dict[str, Any]]) -> dict[int, dict[str, str]]:
    ...
    ], max_tokens=500, enable_thinking=settings.agent_reasoning)
    ...
    if len(items) == len(documents):
        for position, item in enumerate(items):
            ...
            output[documents[position]["index"]] = {
                "type": doc_type if doc_type in _TYPE_CONFIG else "other",
...
async def plan(files: list[FileItem], options: dict | None = None, session: dict | None = None) -> dict:
    ...
    docs = [{"index": index, "text": _truncate(str(ocr_by_name.get(item.get("name"), {}).get("text") or ""))}
    ...
    for index, item in enumerate(raw_files):
        ...
        attachments.append({
            "fileIndex": index, "fileName": item.get("name") or f"file-{index + 1}",
            "documentName": document_name, "componentName": component,
            "target": "new", "needsAddComponent": False,
            "detectedType": component, "category": category,
        })
        summary.append({"fileName": item.get("name"), "type": doc_type,
