...
_OTHER_LABEL = "Tài liệu đăng ký kết hôn"
...
def _merge_all_identities(identity_records: list[dict], slot: tuple[int, str]) -> dict:
    """Gộp mọi giấy tờ tùy thân thành một PDF, bất kể đầu vào là ảnh rời hay PDF hoàn chỉnh."""
    ...
    component_index, component_name = slot
    merged = {
        **primary,
...
async def plan_ket_hon_attachments(
    ...
    if identity_records:
        attachments.insert(identity_insert_at or 0, _merge_all_identities(identity_records, slot))
