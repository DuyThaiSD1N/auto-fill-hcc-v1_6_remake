"""Compact agent process pipeline cho "Cấp lại Chứng chỉ hành nghề thú y".
...
async def _submitter_context(documents: list[dict], options: dict) -> str:
    ...
    if matched:
        ...
        return (
            ...
            f"tài liệu #{idx}.\n"
            "- NGƯỜI ĐỀ NGHỊ (chủ hồ sơ) = người đứng tên Đơn 03.HNTY. Trích vào các field NguoiDeNghi_* "
            f"TỪ Đơn + CCCD của họ, TUYỆT ĐỐI KHÔNG lấy nhân thân người đề nghị từ CCCD người nộp "
            f"(tài liệu #{idx}).\n"
            "- NẾU người nộp CHÍNH LÀ người đề nghị (cùng họ tên hoặc cùng số định danh với Đơn) → chỉ "
            f"1 người: BỎ TRỐNG NguoiNop_*, chỉ trích NguoiDeNghi_* từ CCCD #{idx} + Đơn.\n"
    ...
    return (
        ...
        "NguoiDeNghi_* trích bình thường từ Đơn 03.HNTY / CCCD của người đề nghị. Bỏ trống NguoiNop_*.\n"
        "</nguoi_nop_context>"
    )
