"""Compact agent process pipeline cho "Cấp giấy chứng nhận đăng ký tàu cá, tàu phục vụ nuôi trồng thủy
sản".

HAI vai (thường trùng): CHỦ TÀU (chủ hồ sơ) và NGƯỜI NỘP. context_builder neo NGƯỜI NỘP (tên + CCCD tài
khoản cổng tự đổ vào Phần I) để LLM tách khỏi chủ tàu khi có nộp thay."""

import re
import unicodedata

from app.pipelines._shared.compact_agent import runner
from app.pipelines.cap_gcn_dang_ky_tau_ca.process import mapper
from app.pipelines.cap_gcn_dang_ky_tau_ca.process.prompt import EXTRA_RULES
from app.pipelines.cap_gcn_dang_ky_tau_ca.process.schema import (
    ALIASES,
    ALLOWED,
...
async def _submitter_context(documents: list[dict], options: dict) -> str:
    """Neo NGƯỜI NỘP (tên+CCCD cổng tự đổ từ tài khoản vào Phần I) để LLM tách khỏi CHỦ TÀU."""
    ...
    if matched:
        ...
        return (
            ...
            f"tài liệu #{idx}.\n"
            "- CHỦ TÀU (chủ hồ sơ) = người đứng tên đăng ký tàu (bên mua Hợp đồng / Tờ khai 02a.ĐKT). Trích "
            f"vào các field NguoiDeNghi_* TỪ Tờ khai + CCCD của họ, TUYỆT ĐỐI KHÔNG lấy nhân thân chủ tàu từ "
            f"CCCD người nộp (tài liệu #{idx}).\n"
            "- NẾU người nộp CHÍNH LÀ chủ tàu (cùng họ tên hoặc cùng số định danh) → chỉ 1 người: BỎ TRỐNG "
            f"NguoiNop_*, chỉ trích NguoiDeNghi_* từ CCCD #{idx} + Tờ khai.\n"
    ...
    return (
        ...
        "NguoiDeNghi_* trích bình thường từ Tờ khai 02a.ĐKT / CCCD của chủ tàu. Bỏ trống NguoiNop_*.\n"
        "</nguoi_nop_context>"
    )
