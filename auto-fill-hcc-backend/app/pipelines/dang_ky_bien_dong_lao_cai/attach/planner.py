"""Đính kèm cho [Lào Cai] Đăng ký biến động (1.115671).

LLM đọc OCR từng file → nhãn rút gọn (catalog.py) + nhóm hồ sơ. Nhóm lấy theo VĂN BẢN CĂN CỨ có trong hồ sơ
(thỏa thuận hộ gia đình / cho phép công trình ngầm / cho phép điều chuyển tài sản công / bản án - trọng tài /
xử lý tài sản thế chấp); không có căn cứ nào thì theo nhóm LLM suy ra; vẫn không rõ → nhóm (1). BE đổi nhãn thành:
  - ô fixed-slot của đúng dòng trong nhóm (FE khoanh vùng theo tiêu đề nhóm, tích checkbox dòng rồi bơm tệp);
  - hoặc một dòng "Giấy tờ khác" (target=new, tên = documentName) cho giấy tờ không có dòng trong nhóm: mảnh
    trích đo khi đang ở nhóm (3), đơn/hợp đồng của nhóm khác, hóa đơn, tờ khai thuế, tài liệu khác.
CCCD không phải thành phần hồ sơ → bỏ qua kèm cảnh báo.

Đơn/GCN/bản vẽ/trích đo/ủy quyền có mặt ở MỌI nhóm nên không được dùng để đoán nhóm — chỉ văn bản căn cứ mới
phân biệt được. Chọn sai nhóm thì cán bộ phải bỏ tích cả 6 dòng rồi làm lại, nên thà rơi xuống "Giấy tờ khác".
"""
...
async def _classify_with_llm(documents: list[dict[str, Any]]) -> tuple[str, dict[int, dict[str, str]]]:
    ...
    group = str(parsed.get("group") or "").strip().strip("()")
...
def _pick_group(entries: list[dict], llm_group: str) -> tuple[str, str]:
    """Nhóm hồ sơ = nhóm của VĂN BẢN CĂN CỨ đầu tiên đọc được; không có thì tin LLM; không nữa → nhóm (1)."""
    can_cu = [catalog.CAN_CU_GROUP[e["label"]] for e in entries if e["label"] in catalog.CAN_CU_GROUP]
    if can_cu:
        # Nhiều căn cứ khác nhóm là hồ sơ bất thường → lấy căn cứ xuất hiện đầu tiên, phần còn lại xuống
        # "Giấy tờ khác" để cán bộ nhìn thấy và tự xử lý, thay vì im lặng tích nhầm nhóm.
        return can_cu[0], "can_cu"
    if llm_group in catalog.SECTION_HEADERS:
        return llm_group, "llm"
    return catalog.GROUP_1, "default"
...
def build_plan_items(
    ...
    group, group_source = _pick_group(entries, llm_group)
    ...
    for entry in entries:
        ...
        row = catalog.row_for(entry["label"], group)
        if row:
    ...
    return attachments, warnings, classified, group, group_source
...
async def plan(files: list[FileItem], options: dict | None = None, session: dict | None = None) -> dict:
    ...
    if any(doc["text"].strip() for doc in llm_docs):
        try:
            llm_group, llm_types = await _classify_with_llm(llm_docs)
        except Exception as exc:  # noqa: BLE001
            errors.append(f"attachment_agent: {exc}")
    llm_ms = int((time.monotonic() - t1) * 1000)

    attachments, warnings, classified, group, group_source = build_plan_items(valid_files, llm_types, llm_group)
    ...
    return {
        ...
        "llm_output": {"group": llm_group, **{str(idx): value for idx, value in llm_types.items()}},
        "errors": errors,
    }
