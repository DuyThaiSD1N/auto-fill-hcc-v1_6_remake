"""Đính kèm "[Bắc Ninh] Xóa đăng ký biện pháp bảo đảm bằng QSDĐ, tài sản gắn liền với đất".

LLM phân loại là chính → nhãn rút gọn (catalog.py) → BE map sang componentName (mã KQ) + slotKey hoặc ô
đính kèm BỔ SUNG. Rule từ khóa/tên file chỉ fallback khi OCR rỗng hoặc LLM không chắc.
"""
import re
import time
...
def _label_by_keywords(ocr_text: str) -> str | None:
    ...
    if not h.strip():
        return None
    # GCN thật (sổ đỏ) có "số vào sổ" — ưu tiên cả khi file gộp.
    if "so vao so" in h and "giay chung nhan" in h:
        return "gcn"
    if "phieu yeu cau xoa dang ky" in h or ("xoa dang ky" in h and "bien phap bao dam" in h):
        return "phieu_yc_03a"
    if "mau so 03a" in h and "dang ky" in h:
        return "phieu_yc_03a"
    if "hop dong the chap" in h:
        return "hop_dong_the_chap"
...
def _label_by_filename(name: str) -> str | None:
    ...
    if "can cuoc" in h or "cccd" in h or "cmnd" in h or "cmt" in h:
        return "cccd"
    if "phieu" in h and ("xoa" in h or "03a" in h):
        return "phieu_yc_03a"
    if "the chap" in h and "hop dong" in h:
        return "hop_dong_the_chap"
