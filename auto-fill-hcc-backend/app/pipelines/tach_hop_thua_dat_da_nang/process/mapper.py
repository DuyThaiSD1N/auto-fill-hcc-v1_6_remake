"""Map compact source facts → Form.io data[...] fields cho "Tách thửa đất hoặc hợp thửa đất" (cổng Đà
Nẵng). CÙNG contact block với #75/chuyen_muc_dich — chỉ khác _PROC_TITLE.

HAI vai trong 1 panel:
- CHỦ HỒ SƠ (chủ đất đề nghị tách/hợp thửa) → data[ownerFullname] (+ data[organization] khi tổ chức).
...
from app.pipelines.tach_hop_thua_dat_da_nang.process.schema import UI_COMP_BY_NAME

# Văn bản "Nội dung yêu cầu giải quyết" ghép: "{tên chủ hồ sơ} ĐỀ NGHỊ GIẢI QUYẾT {tên thủ tục}".
_PROC_TITLE = "Tách thửa đất hoặc hợp thửa đất"
...
def enrich(fields: list[dict], options: dict | None = None) -> tuple[list[dict], list[str]]:
    ...
    if not owner_name:
        warnings.append("Thiếu tên chủ hồ sơ (chủ đất đề nghị tách/hợp thửa).")
    ...
    # Nội dung yêu cầu: chép ĐẦY ĐỦ mô tả tách/hợp thửa từ Đơn/Bản vẽ (GIỮ xuống dòng danh sách thửa con);
    # CHỈ khi trống mới fallback câu khung "{tên chủ hồ sơ} ĐỀ NGHỊ GIẢI QUYẾT {tên thủ tục}".
