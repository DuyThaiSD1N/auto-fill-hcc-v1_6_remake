...
async def ws_tts(ws: WebSocket, lang: str = "vi", voice: str = ""):
    ...
    upstream_url = (
        ...
        if lang == "hmong" and settings.tts_ws_url_hmong
        else settings.tts_ws_url
    )
    if not upstream_url or not settings.tts_api_key:
    ...
    try:
        ...
        # không phải lỗi ASGI cần in nguyên traceback.
        try:
