...
async def test_tthn_attachment_plan_routes_identity_to_new_component(monkeypatch):
    ...
    async def fake_chat(messages, max_tokens, enable_thinking):
        ...
        assert documents == [{"index": 0, "ocrText": "CĂN CƯỚC CÔNG DÂN"}]
        return json.dumps({"documents": [
            {"index": 0, "type": "identity", "title": "Căn cước công dân"}
        ]})
...
async def test_tthn_attachment_plan_routes_condition_documents_to_existing_rows(monkeypatch):
    ...
    async def fake_chat(messages, max_tokens, enable_thinking):
        documents = json.loads(messages[1]["content"])["documents"]
        types = [
            ("divorce_or_death_proof", "Quyết định ly hôn"),
            ("foreign_divorce_note", "Trích lục ghi chú ly hôn"),
...
def test_tthn_attachment_prompt_contains_all_ocr_and_no_file_name():
    ...
    assert payload == {"documents": [{
        "index": 0,
        "ocrText": "TỜ KHAI CẤP GIẤY XÁC NHẬN TÌNH TRẠNG HÔN NHÂN",
    }]}
...
async def test_tthn_llm_failure_falls_back_all_files_without_retry(monkeypatch):
    async def fake_ocr_per_file(_files):
        return [
