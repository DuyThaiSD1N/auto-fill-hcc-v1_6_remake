...
from app.pipelines.khai_sinh_dang_ky_lai.attach import planner_v2 as dang_ky_lai_khai_sinh
from app.pipelines.khai_sinh_dang_ky_lai.attach.prompt_v2 import SYSTEM_PROMPT, build_user_prompt
...
async def test_dang_ky_lai_khai_sinh_merges_all_cccds_without_mixing_faces(monkeypatch):
    ...
    assert first["documentName"] == "Căn cước công dân"
    assert [segment["fileIndex"] for segment in first["sourceSegments"]] == [0, 2]
    assert second["target"] == "new"
    assert [segment["fileIndex"] for segment in second["sourceSegments"]] == [1, 3]
