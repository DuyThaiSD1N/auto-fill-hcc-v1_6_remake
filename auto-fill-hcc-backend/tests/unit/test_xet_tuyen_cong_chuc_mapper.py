"""Mapper tests for Xét tuyển công chức."""

from app.pipelines._shared.compact_agent import prompt as compact_prompt
from app.pipelines.xet_tuyen_cong_chuc import process as agent
from app.pipelines.xet_tuyen_cong_chuc.process import mapper
from app.pipelines.xet_tuyen_cong_chuc.process.prompt import EXTRA_RULES
from app.pipelines.xet_tuyen_cong_chuc.process.schema import FIELDS
...
def test_xet_tuyen_cong_chuc_registered():
    key = "xet-tuyen-cong-chuc"
    proc = get_procedure(key)

    assert proc
    assert proc["label"] == "Xét tuyển công chức"
...
def test_xet_tuyen_cong_chuc_maps_full_sample():
    ...
    current_district = [f for f in out if f["name"] == "data[district]" and f.get("occurrence") == 1][0]
    assert current_province["value"] == "Thành phố Hà Nội"
    assert current_district["value"] == "Phường Láng"
    ...
    assert d["data[DataGrid2][0][textField1]"] == "Phòng kinh tế xã Nậm Cuổi"
    assert d["data[DataGrid2][0][textField2]"] == "Nguyện vọng 1"


def test_xet_tuyen_cong_chuc_does_not_overwrite_mismatched_requester():
    out, warnings = mapper.enrich(
        _sample_fields(),
...
def test_xet_tuyen_cong_chuc_prompt_requires_long_schema():
...
def test_xet_tuyen_cong_chuc_occurrence_survives_response_model_dump():
