...
def test_nhan_cha_me_con_repairs_parent_block_mistakenly_copied_from_child():
    """Tờ khai: "Đề nghị công nhận [Hoàng Gia Minh] ... Là Con của [Hoàng Quốc Trung]" — LLM đọc
    nhầm hướng nhãn "Là Con của" nên gán CẢ Parent_* lẫn Child_* cùng trỏ về đứa trẻ (Mục II trên
    cổng bị điền trùng hệt Mục III, thiếu hẳn CCCD của người cha). Vì người yêu cầu tự khai quan hệ
    là "Cha" nên Mục II (cha/mẹ) phải được vá lại bằng chính nhân thân người yêu cầu (Hoàng Quốc Trung)."""
    fields = [
        _field("Requester_FullName", "Hoàng Quốc Trung"),
        _field("Requester_BirthDate", "17/03/2008"),
        _field("Requester_IdNumber", "027208011967"),
        _field("Requester_IdIssueDate", "04/05/2022"),
        _field("Requester_IdIssuePlace", "Cục Cảnh sát quản lý hành chính về trật tự xã hội"),
        _field(
            "Requester_ResidenceDomestic",
            {"quocGia": "Việt Nam", "tinh": "Bắc Ninh", "xa": "Nam Sơn", "diaChi": "Tổ dân phố Lâm Dương"},
        ),
        _field("Requester_RelationshipToRecognized", "Cha"),
        # Parent_* bị LLM gán NHẦM sang dữ liệu của con (bug đang sửa).
        _field("Parent_FullName", "Hoàng Gia Minh"),
        _field("Parent_BirthDate", "12/07/2026"),
        _field("Parent_Gender", "Nam"),
        _field("Parent_Ethnicity", "Kinh"),
        _field(
            "Parent_ResidenceDomestic",
            {"quocGia": "Việt Nam", "tinh": "Bắc Ninh", "xa": "Nam Sơn", "diaChi": "Tổ dân phố Lâm Dương"},
        ),
        _field("Child_FullName", "Hoàng Gia Minh"),
        _field("Child_BirthDate", "12/07/2026"),
        _field("Child_Gender", "Nam"),
        _field("Child_BirthDocumentType", "Giấy chứng sinh"),
        _field("Child_BirthDocumentNumber", "02648.GCS 27174.26"),
    ]

    d = _values(mapper.enrich(fields, {}))

    # Mục II (cha/mẹ) phải là NGƯỜI YÊU CẦU (Hoàng Quốc Trung), KHÔNG phải đứa trẻ.
    assert d["HotenA"] == "HOÀNG QUỐC TRUNG"
    assert d["ngaysinhA"] == "17/03/2008"
    assert d["sodinhdanhA"] == "027208011967"
    assert d["sogiaytodinhdanhA"] == "027208011967"
    assert d["ngaycapA"] == "04/05/2022"
    assert d["NoiCapA"] == "Cục Cảnh sát quản lý hành chính về trật tự xã hội"
    assert d["noicutruA_TrongNuoc"]["diaChi"] == "Tổ dân phố Lâm Dương"
    # Mục III (con) vẫn đúng là Hoàng Gia Minh, không bị đụng vào.
    assert d["hotenB"] == "HOÀNG GIA MINH"
    assert d["ngaysinhB"] == "12/07/2026"


def test_nhan_cha_me_con_keeps_parent_block_when_genuinely_different_person():
    """Cha/mẹ khác người con (ca bình thường) → KHÔNG được ghi đè bằng dữ liệu người yêu cầu."""
    fields = [
        _field("Requester_FullName", "Hoàng Quốc Trung"),
        _field("Requester_IdNumber", "027208011967"),
        _field("Requester_RelationshipToRecognized", "Cha"),
        _field("Parent_FullName", "Hoàng Quốc Trung"),
        _field("Parent_IdNumber", "027208011967"),
        _field("Child_FullName", "Hoàng Gia Minh"),
        _field("Child_IdNumber", ""),
    ]
