"""[Lào Cai] 1.115667 — hai chế độ xác định NGƯỜI NỘP.

Cổng đổ sẵn Họ tên/Số căn cước/địa chỉ/Di động của tài khoản định danh vào khối người nộp, rồi gửi
Họ tên + Số căn cước + Ngày sinh sang CSDL quốc gia dân cư để xác thực trước khi cho nộp. Hệ quả:
chế độ theo tài khoản chỉ bù nhân thân của CHÍNH người đó và không đụng phần cổng đã điền (sửa ô
"Họ và tên" là cổng xoá trắng Di động + Số căn cước); chế độ theo tờ khai phải ghi đè CẢ khối.
"""

from pathlib import Path

from app.pipelines.cap_GCN_nhan_chuyen_nhuong.process import mapper
from app.pipelines.cap_GCN_nhan_chuyen_nhuong.process.schema import UI_COMP_BY_NAME

_ANCHOR = {"applicantFullname": "NGUYỄN VĂN NAM", "applicantIdentityNumber": "001085000123"}

# Hồ sơ mẫu: bên nhận chuyển nhượng (chủ hồ sơ) là bà Trần Thị Bé, người đi nộp là con trai có thẻ
# căn cước riêng trong hồ sơ.
_FACTS = [
    ...
    {"name": "ChuHoSo_NgayCap", "value": "20/05/2021"},
...
def test_che_do_tai_khoan_khong_lay_the_cua_chu_ho_so():
    ...
    assert values["ChuHoSo_soCMNDChuHoSo"] == "034057017088"
...
def test_che_do_to_khai_ghi_de_ca_khoi_theo_chu_ho_so():
    """Người nộp theo tờ khai là NGƯỜI KHÁC với tài khoản → phải ghi cả Họ tên/Số căn cước/địa chỉ."""
    ...
    assert any("không đúng với tài khoản" not in w for w in warnings)
...
def test_che_do_to_khai_khong_canh_bao_khi_trung_tai_khoan():
    fields, warnings = mapper.enrich(_FACTS, {
        "formContext": {"applicantFullname": "TRẦN THỊ BÉ", "applicantIdentityNumber": "034057017088"},
        "submitterMode": "owner_as_submitter",
    })

    assert _values(fields)["CongDan_tenCongDan"] == "TRẦN THỊ BÉ"
    assert not warnings
...
def test_popup_gui_form_context_cho_thu_tuc_nay():
    ...
    assert 'cfg.key === "dang-ky-cap-gcn-nhan-chuyen-nhuong-du-an-bat-dong-san-lao-cai"' in block
