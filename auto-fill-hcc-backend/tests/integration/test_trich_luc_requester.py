...
_SON = {
    "Cccd_HoTen": "ĐOÀN TIẾN MẠNH",
    "Cccd_SoDinhDanh": "012213008048",
    "Cccd_NgayCap": "03/09/2024",
    "Cccd_NoiCap": "Bộ Công an",
}
_MOTHER = {
    "Cccd_HoTen": "NGUYỄN THỊ THANH NGA",
    "Cccd_SoDinhDanh": "001181030693",
    "Cccd_NgayCap": "25/05/2026",
    "Cccd_NoiCap": "Bộ Công an",
}
_HOTICH = {
...
# Chỉ up 1 CCCD (không có giấy hộ tịch): người đó = NGƯỜI ĐƯỢC ĐĂNG KÝ (mục II).
_DECEASED = {
    "Cccd_HoTen": "NGUYỄN ĐỨC TOÀN",
    "Cccd_SoDinhDanh": "034072010036",
    "Cccd_NgaySinh": "20/03/1972",
    "Cccd_GioiTinh": "Nam",
    "Cccd_NgayCap": "28/04/2023",
    "Cccd_NoiCuTru": {"quocGia": "Việt Nam", "tinh": "Lai Châu", "xa": "Đoàn Kết", "diaChi": "Tổ 10"},
}
...
def test_only_cccd_no_context_fills_ndk_not_requester():
    ...
    out = _by_name(mapper.enrich(_fields(_DECEASED)))
    assert "HoVaTenC" not in out
    ...
    assert out["NDK_NoiCuTru_TrongNuoc"]["value"]["xa"] == "Đoàn Kết"


def test_only_cccd_matches_formcontext_fills_both():
    """Chỉ 1 CCCD, tên+số khớp người đăng nhập → điền CẢ mục II lẫn mục I (người yêu cầu cũng là chính người đó)."""
    out = _by_name(mapper.enrich(
        _fields(_DECEASED),
