"""Compact agent điều chỉnh đất đai: OCR text -> CCCD/GCN facts -> DOM UI fields."""
...
from app.pipelines.dieu_chinh_dat_dai import process as agent
from app.pipelines.dieu_chinh_dat_dai.process.prompt import EXTRA_RULES
from app.pipelines.dieu_chinh_dat_dai.process.schema import FIELDS
...
@respx.mock
async def test_dieu_chinh_dat_dai_compact_agent_derives_ui_fields(monkeypatch):
...
@respx.mock
async def test_dieu_chinh_dat_dai_falls_back_to_ocr_gcn_serial(monkeypatch):
    monkeypatch.setattr(settings, "openai_api_key", "")
    ocr_text = "\n".join([
        "GIẤY CHỨNG NHẬN",
        "QUYỀN SỬ DỤNG ĐẤT",
...
def test_dieu_chinh_dat_dai_prompt_locks_gcn_serial():
...
def test_registry_uses_dieu_chinh_dat_dai_compact_agent_mode():
    proc = get_procedure("dieu-chinh-dat-dai")

    assert get_pipeline("dieu-chinh-dat-dai") is agent.run
