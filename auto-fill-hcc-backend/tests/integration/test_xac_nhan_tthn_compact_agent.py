...
def _requester_block(**overrides):
    """Tờ khai có khối "người yêu cầu" riêng + CCCD trong hồ sơ là của NGƯỜI ĐI NỘP."""
    values = {
        "Cccd_HoTen": "NGUYỄN ĐÌNH THÀNH",
        "Cccd_SoDinhDanh": "024089012690",
        "Cccd_NgaySinh": "13/03/1989",
        "Cccd_NgayCap": "29/11/2022",
        "ToKhaiYeuCau_HoTen": "PHẠM THỊ XUÂN",
        "ToKhaiYeuCau_SoDinhDanh": "024153007282",
        "ToKhaiYeuCau_NgayCapGiayTo": "25/08/2021",
        "ToKhaiYeuCau_QuanHe": "Bản thân",
        "ToKhai_HoTen": "PHẠM THỊ XUÂN",
        "ToKhai_SoDinhDanh": "024153007282",
        "ToKhai_NgaySinh": "27/03/1953",
    }
    values.update(overrides)
    return [{"name": k, "comp": "x-input", "value": v} for k, v in values.items() if v is not None]


def test_xac_nhan_tthn_fills_requester_birthdate_from_declaration():
    mapped = mapper.enrich(_requester_block(ToKhaiYeuCau_NgaySinh="27/03/1953"))
    d = {f["name"]: f["value"] for f in mapped}

    assert d["HoVaTenC"] == "PHẠM THỊ XUÂN"
    assert d["NgaySinhC"] == "27/03/1953"


def test_xac_nhan_tthn_borrows_section_two_birthdate_for_same_requester():
    # Tờ khai không tách riêng ngày sinh người yêu cầu, nhưng mục I và mục II là CÙNG một người
    # (trùng số định danh) → lấy ngày sinh ở Section II, KHÔNG mượn CCCD của người đi nộp.
    mapped = mapper.enrich(_requester_block())
    d = {f["name"]: f["value"] for f in mapped}

    assert d["NgaySinhC"] == "27/03/1953"


def test_xac_nhan_tthn_leaves_requester_birthdate_empty_when_unknown():
    # Người yêu cầu KHÁC người được cấp và tờ khai không ghi ngày sinh của họ: bỏ trống còn hơn
    # điền ngày sinh trên CCCD của người đi nộp.
    mapped = mapper.enrich(_requester_block(
        ToKhai_HoTen="TRẦN VĂN A",
        ToKhai_SoDinhDanh="001199000111",
        ToKhaiYeuCau_QuanHe="là con đẻ",
    ))
    d = {f["name"]: f["value"] for f in mapped}

    assert d["HoVaTenC"] == "PHẠM THỊ XUÂN"
    assert d.get("NgaySinhC") in (None, "")


def test_xac_nhan_tthn_prompt_requires_requester_birthdate():
    system_prompt = compact_prompt.build_system_prompt(FIELDS, EXTRA_RULES)

    assert "ToKhaiYeuCau_NgaySinh" in system_prompt
