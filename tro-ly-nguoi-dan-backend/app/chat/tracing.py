...
Cột `name` GIỮ = phường/xã của PHIÊN (conv.location, đổi được trên card) chứ không
phải phường của acc — trang quản lý lọc/thống kê theo nơi làm thủ tục thật.
...
Mọi hàm đều best-effort: lỗi ghi vết không được chặn luồng điền của bà con.
"""
import logging
import uuid
...
def _ocr_label() -> str:
    return ocr.resolved_label(None if settings.ocr_by_tiengnoi else "raw")


async def _save_request(request_id: str, created_at: datetime, conv: dict,
                        procedure_key: str, files: list[dict]) -> str | None:
    ...
    return await requests_repo.create_request(
...
async def record_process(conv: dict, procedure_key: str, proc: dict, files: list[dict],
                         result: dict, pipe_ms: int) -> str | None:
    ...
    try:
        ...
        doc_id = await _save_request(request_id, created_at, conv, procedure_key, files)
        ...
        await traces_repo.create_trace(
            ...
            stats=stats, created_at=created_at,
        )
        return request_id
...
async def record_attach(conv: dict, procedure_key: str, proc: dict, files: list[dict],
    ...
    try:
        ...
        attachments = [
            ...
            for it in plan
        ] or [{"name": f.get("name", ""), "role": ""} for f in files]
        uid, uname = _identity(conv)
        await traces_repo.create_trace(
            request_id=request_id, user_id=uid, username=uname,
            name=_location_name(conv),
            ...
            fields_count=len(plan), status="done", stats=stats, created_at=created_at,
        )
        return request_id
...
async def record_error(conv: dict, procedure_key: str, kind: str, error: str) -> None:
    ...
    try:
        ...
        uid, uname = _identity(conv)
        await traces_repo.create_trace(
            request_id=uuid.uuid4().hex, user_id=uid, username=uname,
            name=_location_name(conv), kind=kind,
            procedure=procedure_key, procedure_label=proc_label,
            ocr_provider=_ocr_label(), ocr_text="", llm_output=None,
            fields_count=0, status="error", error_code=str(error)[:200],
            created_at=datetime.now(timezone.utc),
        )
    except Exception as e:  # noqa: BLE001
