...
# Quan hệ trên TỜ KHAI (fold dấu) → đúng NHÃN option radio NYC_QuanHe trên form
# (fillRadio khớp theo text nhãn — không dùng suffix id). "ba"/"bà" trần CỐ TÌNH bỏ:
# fold trùng nhau ("Ba"=bố Nam bộ vs "Bà") — tick nhầm tệ hơn bỏ trống.
_QUANHE_OPTIONS = {
    "ban than": "Bản thân", "chinh minh": "Bản thân",
    # "Tự khai"/"Tư khai" (OCR rớt dấu) trên tờ khai = tự làm cho MÌNH → option "Bản thân".
    "tu khai": "Bản thân", "tu ban than": "Bản thân",
    "con": "Con Đẻ", "con de": "Con Đẻ", "con ruot": "Con Đẻ",
    "con nuoi": "Con nuôi",
    "vo": "Vợ", "chong": "Chồng",
    "bo": "Bố Đẻ", "cha": "Bố Đẻ", "bo de": "Bố Đẻ", "cha de": "Bố Đẻ",
    "bo ruot": "Bố Đẻ", "cha ruot": "Bố Đẻ",
    "bo nuoi": "Bố nuôi", "cha nuoi": "Bố nuôi",
    "me": "Mẹ đẻ", "me de": "Mẹ đẻ", "me ruot": "Mẹ đẻ",
    "me nuoi": "Mẹ nuôi",
    "ba noi": "Bà", "ba ngoai": "Bà",
    "ong": "Ông", "ong noi": "Ông", "ong ngoai": "Ông",
    "anh": "Anh ruột", "anh ruot": "Anh ruột", "anh trai": "Anh ruột",
    "chi": "Chị ruột", "chi ruot": "Chị ruột", "chi gai": "Chị ruột",
    "chau": "Cháu ruột", "chau ruot": "Cháu ruột", "chau noi": "Cháu ruột", "chau ngoai": "Cháu ruột",
    "khac": "Khác",
}
...
def enrich(fields: list[dict], options: dict | None = None) -> list[dict]:
    ...
    # Mặc định VIỀN VÀNG cho khối cư trú (giá trị đoán, KHÔNG từ giấy tờ). `seen` đảm bảo KHÔNG đè
    # lên giá trị THẬT đã fill ở trên — chỉ bù khi tờ khai/CCCD không cho nơi cư trú.
    ...
    add("NDK_LoaiCuTru", "Thường trú", default=True)
    add("NDK_NoiCuTru", "1", default=True)
    add("NDK_NoiCuTru_TrongNuoc", {"quocGia": "Việt Nam"}, default=True)

    # (5) Quan hệ với người được cấp bản sao — đọc từ TỜ KHAI; không khớp option nào thì
    # BỎ TRỐNG (không tick bừa "Khác" — người dân tự chọn).
    quanhe = _quanhe_option(values.get("CopyRequest_QuanHe"))
    if quanhe:
        add("NYC_QuanHe", quanhe)

    # Số lượng bản sao: lấy từ tờ khai nếu ghi rõ, không có thì mặc định 3 bản (viền vàng).
    # eForm cổng mới KHÔNG có radio Có/Không cấp bản sao.
    copy_quantity = _copy_quantity(values.get("CopyRequest_Quantity"))
    if copy_quantity:
        add("SoLuong", copy_quantity)
    else:
        add("SoLuong", "3", default=True)
