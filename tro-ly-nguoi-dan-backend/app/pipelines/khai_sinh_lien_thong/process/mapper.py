...
def enrich(fields: list[dict]) -> list[dict]:
    ...
    if ct01_ten or ct01_sdd:
        ...
        if is_bo:
            ...
            add("LoaiThanNhanXacNhan", "1")
            add("LoaiChuSoHuuChoO", "1")
            seen.add("DkttIsTtMe")
        elif is_me:
            add("DkttIsTtMe", True)
            add("DkttMaQuanHe", "Con đẻ")
            add("LoaiThanNhanXacNhan", "2")
            add("LoaiChuSoHuuChoO", "2")
            seen.add("DkttIsTtBo")
        else:  # chủ hộ là người khác → điền tay 3 ô
            # Không đủ căn cứ chọn bố/mẹ ở hai radio owner/confirmer; để người dùng chọn theo
            # giấy tờ thay vì để STATIC_DEFAULTS suy ra sai là bố.
            seen.update({
                "DkttIsTtBo",
                "DkttIsTtMe",
                "LoaiThanNhanXacNhan",
