"""Đính kèm bước 3 cho thủ tục "Đăng ký giám hộ".
...
from app.pipelines.dang_ky_giam_ho.attach import prompt
...
def _rule_doc_type(text: str) -> str:
    ...
    if "to khai dang ky giam ho" in haystack:
        return _DOC_PAPER_DECLARATION

    if "giay xac nhan tinh trang hon nhan" in haystack and "dang ky giam ho" not in haystack:
        return _DOC_SKIP
...
def _rule_document_name(doc_type: str, text: str) -> str:
    ...
    if doc_type == _DOC_CONDITION:
        if _has_any(haystack, ("can cuoc cong dan", "the can cuoc", "citizen identity card", "idvnm")):
            return "Căn cước công dân"
        if "ban cam doan" in haystack:
            return "Bản cam đoan đủ điều kiện giám hộ"
        if _has_any(haystack, ("giay chung nhan quyen su dung dat", "quyen su dung dat")):
...
def build_plan_items(
    ...
    condition_entries = [entry for entry in resolved if entry["docType"] == _DOC_CONDITION]
    ...
    for entry in sorted(resolved, key=lambda item: item["idx"]):
        ...
        if doc_type == _DOC_CONDITION:
            ...
            primary = item["fileIndex"]
            for condition in condition_entries:
                detail_name = _resolve_document_name(
                    condition["file"],
                    condition["docType"],
                    condition["nameSource"],
                    used_names,
                )
                classified.append({
                    "fileName": condition["fileName"],
                    "docType": condition["docType"],
                    "documentName": detail_name,
                    "target": "existing",
                    "componentIndex": 3,
                    "mergedIntoFileIndex": primary,
