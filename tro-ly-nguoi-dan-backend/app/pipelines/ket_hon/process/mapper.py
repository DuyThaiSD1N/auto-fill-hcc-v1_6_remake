...
def enrich(fields: list[dict]) -> list[dict]:
    ...
    def add_person(src: str, dst: str, declaration_area_name: str) -> None:
        ...
        elif not is_foreign(nationality):
            # CCCD gắn chip không in mục dân tộc → mặc định "Kinh" (đại đa số dân số),
            # tô vàng để người dân tự rà; người nước ngoài thì không áp.
            add(f"DanToc{dst}", "Kinh", default=True)
    ...
    if out:
        # Form này là tờ khai đăng ký kết hôn MỚI (đăng ký lại có form riêng) → loại đăng
        # ký luôn "Đăng ký lần đầu". Radio cổng khớp theo suffix id (loaiDangKy-1,
        # CapBanSao-YES/-NO) nên giá trị cấp bản sao dùng YES/NO chứ không phải Có/Không.
        add("loaiDangKy", "1", default=True)
        # Cấp bản sao: ưu tiên tờ khai (số lượng dương = "Có" + điền input SoLuong);
        # tờ khai không ghi gì → mặc định "Không" (tô vàng) — ai cần thì tự tích lại.
        copy_quantity = _positive_copy_quantity(values.get("CopyRequest_Quantity"))
        wants = _copy_value(values.get("CopyRequest_WantsCopy"))
        if copy_quantity:
            add("CapBanSao", "YES")
            add("SoLuong", copy_quantity)
        elif wants:
            add("CapBanSao", "YES" if wants == "Có" else "NO")
        else:
            add("CapBanSao", "NO", default=True)
    return out
