...
# Cổng React mới tách "Văn bản ủy quyền" thành hàng 5 riêng (bản cũ gộp chung hàng 4).
_ROW_5_COMPONENT = "Văn bản ủy quyền theo quy định của pháp luật"
...
def _canonical_type(value: str) -> str:
    ...
    if text in {"previous marital status certificate", "previous certificate",
                "previous marital status certificate or authorization"}:
        # Chuỗi gộp cũ vẫn nhận (LLM cache/prompt cũ) → coi là giấy XN TTHN đã cấp.
        return "previous_marital_status_certificate"
    if text in {"authorization", "authorization letter"}:
