...
def enrich(fields: list[dict]) -> list[dict]:
    ...
    def add_person(src: str, dst: str) -> None:
        ...
        if foreign:
            # Giấy tờ nước ngoài: loại = "Giấy tờ khác..." + ô tên giấy tờ = tên giấy thật.
            # Form thật đặt name TenGiayTo_* (x-select-area) — không phải NhapTenGiayTo raw.
            add(f"LoaiGiayToDinhDanh_{dst}", FOREIGN_DOC_TYPE)
            add(f"TenGiayTo_{dst}", values.get(f"{src}_TenGiayTo") or "Chứng minh thư")
    ...
    # Mặc định chung của form (viền vàng, như ket_hon nội địa): đăng ký lần đầu (radio -1),
    # không đề nghị cấp bản sao (radio -NO).
