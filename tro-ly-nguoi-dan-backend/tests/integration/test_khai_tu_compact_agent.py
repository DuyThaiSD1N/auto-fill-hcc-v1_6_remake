...
async def test_khai_tu_compact_agent_derives_ui_fields(monkeypatch):
    ...
    respx.post(settings.ocr_raw_base_url.rstrip("/") + "/api/v1/ocr/raw").mock(
        return_value=httpx.Response(200, json={"fullText": gbt_page})
    )
    fields = {
...
async def test_khai_tu_phan_vai_loai_cccd_lech_whitelist(monkeypatch):
    ...
    _disable_external_fallbacks(monkeypatch)
    respx.post(settings.ocr_raw_base_url.rstrip("/") + "/api/v1/ocr/raw").mock(
        return_value=httpx.Response(200, json={"fullText": "CĂN CƯỚC CÔNG DÂN 040203015844"})
    )
    role_text = (
...
def test_registry_khai_tu_entry():
    ...
    assert set(docs) == {"cccd", "bao_tu", "to_khai"}
    assert not docs["cccd"].get("optional") and docs["cccd"]["sides"] == 2
