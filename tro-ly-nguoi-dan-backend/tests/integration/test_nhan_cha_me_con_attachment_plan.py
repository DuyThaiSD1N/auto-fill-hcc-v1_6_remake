...
def test_registry_nhan_cha_me_con_tro_ly_entry():
    """Entry bản tro-ly: ẩn khỏi card, cổng React mới, checklist QR + classify route."""
    ...
    assert proc["keKhaiUrl"].endswith("019d2bfd-3fa6-722f-ac1a-e949c8ce3418")
    docs = {r["key"]: r for r in proc["requiredDocs"]}
    assert set(docs) == {"to_khai", "cccd", "ho_tich", "khac"}
    assert not docs["to_khai"].get("optional")     # tờ khai = nguồn chính quan hệ/loại xác nhận
    assert docs["cccd"]["sides"] == 4              # tối đa 2 người × 2 mặt

    # Tờ khai nhận cha mẹ con ("đăng ký") → to_khai; giấy khai sinh con → ho_tich.
    assert classify_text("TỜ KHAI ĐĂNG KÝ NHẬN CHA, MẸ, CON")["doc_type"] == "to_khai"
    info_ks = classify_text("GIẤY KHAI SINH Số 123/2020")
    assert route_to_slot(info_ks, proc["requiredDocs"], [], None)[0] == "ho_tich"
    # Giấy chứng sinh: KHÔNG có slot riêng → fallthrough vào "khác" (không bị chối).
    info_cs = classify_text("GIẤY CHỨNG SINH Số 45 Trạm y tế xã")
    assert route_to_slot(info_cs, proc["requiredDocs"], [], None)[0] == "khac"
