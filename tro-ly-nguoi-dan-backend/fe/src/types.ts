...
// ── Nhật ký chấp thuận PDPL (tab Chấp thuận — chỉ admin) ──
export interface ConsentListItem {
  id: string;              // mã nhật ký TLND-xxxx
  at?: string | null;
  at_display?: string;
  procedure_key?: string;
  procedure_label?: string;
  accepted: boolean;
  method?: string;         // chip (bấm nút) | verbal (nói/gõ)
  version?: string;
  location?: { province?: string; ward?: string };
  auth_username?: string;
  // Chủ thể dữ liệu = tài khoản VNeID trên cổng: ưu tiên CCCD, chỉ tên khi cổng không lộ số,
  // cả hai null = ghi nhận theo mã phiên.
  principal_cccd?: string | null;
  principal_name?: string | null;
  conversation_id?: string;
  has_pdf: boolean;        // chỉ lượt đồng ý có PDF biên bản
}
