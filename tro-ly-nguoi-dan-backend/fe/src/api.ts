...
// --- Danh mục tỉnh/xã (public — đổ dropdown chọn nơi ở form tài khoản) ---
export interface Province {
  text: string; // tên đầy đủ chính thức, vd "Tỉnh Lai Châu"
  slug: string;
  name: string; // tên ngắn, vd "Lai Châu" — khớp giá trị account nhập tay thời trước
}

export function getProvinces(): Promise<{ provinces: Province[] }> {
  return request<{ provinces: Province[] }>(`/api/v1/provinces`);
}

export function getWards(slug: string): Promise<{ slug: string; province: string; communes: string[] }> {
  return request<{ slug: string; province: string; communes: string[] }>(
    `/api/v1/wards?slug=${encodeURIComponent(slug)}`,
  );
}
...
// --- Nhật ký chấp thuận PDPL (admin) ---
export function listConsents(
  page = 1,
  pageSize = 20,
  accepted: "" | "true" | "false" = "",
): Promise<ConsentListResp> {
  const params = new URLSearchParams({ page: String(page), pageSize: String(pageSize) });
  if (accepted) params.set("accepted", accepted);
  return request<ConsentListResp>(`/api/v1/consents?${params.toString()}`);
}

// PDF biên bản chấp thuận — blob kèm Authorization (mở tab mới bằng object URL).
export function fetchConsentPdf(logId: string): Promise<Blob> {
  return requestBlob(`/api/v1/consents/${encodeURIComponent(logId)}/pdf`);
}
