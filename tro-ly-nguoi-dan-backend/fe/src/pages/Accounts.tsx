...
import { createUser, deleteUser, getProvinces, getWards, listUsers, updateUser } from "../api";
...
// "Phường Tân Phong" → "Tân Phong": để khớp giá trị tài khoản cũ nhập tay tên trần.
const stripWardType = (w: string) => w.replace(/^(Phường|Xã|Đặc khu)\s+/i, "");
...
export default function Accounts({ user, onLogout, view, onNavigate }: Props) {
  ...
  // Khớp cả tên đầy đủ ("Tỉnh Lai Châu") lẫn tên ngắn nhập tay thời trước ("Lai Châu").
  const provOf = useCallback(
    (tinh: string) =>
      provinces.find((p) => p.text === tinh) ?? provinces.find((p) => p.name === tinh),
    [provinces],
  );

  // Mở form với giá trị cũ dạng ngắn → nâng lên tên đầy đủ để select nhận diện.
  useEffect(() => {
    if (!form?.tinh || provinces.length === 0) return;
    const p = provOf(form.tinh);
    if (p && p.text !== form.tinh) setForm((f) => (f ? { ...f, tinh: p.text } : f));
  }, [form?.tinh, provinces, provOf]);

  // Đổi tỉnh → nạp danh sách xã của tỉnh đó.
  useEffect(() => {
    const p = form?.tinh ? provOf(form.tinh) : undefined;
    if (!p) {
      setWards([]);
      return;
    }
    let alive = true;
    getWards(p.slug)
      .then((r) => alive && setWards(r.communes))
  ...
  // Xã cũ nhập tay tên trần ("Tân Phong") → nâng lên tên đầy đủ khi danh sách xã về.
  useEffect(() => {
    if (!form?.xa || wards.length === 0 || wards.includes(form.xa)) return;
    const m = wards.find((w) => stripWardType(w) === form.xa);
    if (m) setForm((f) => (f ? { ...f, xa: m } : f));
  }, [wards, form?.xa]);
          ...
          <form className="modal-card" onClick={(e) => e.stopPropagation()} onSubmit={submitForm}>
            ...
            <div className="form-row">
              <label>
                ...
                <SearchSelect
                  value={form.tinh}
                  options={provinces.map((p) => p.text)}
                  placeholder="Gõ để tìm tỉnh/thành…"
                  onChange={(v) => setForm({ ...form, tinh: v, xa: "" })}
                />
              </label>
              <label>
                Xã / Phường
                <SearchSelect
                  ...
                  disabled={!provOf(form.tinh)}
                  onChange={(v) => setForm({ ...form, xa: v })}
                />
              </label>
            </div>
